var collaboration = {
  ...
  init: function (d, c) {
    ...
    $(document).on('click', '.colla-user, .colla-team-user > div', function (g) {
      var f = $(this).attr('uid'),
        h = $(this).text();
      ...
      setVal: function (g) {
        ...
        for (var f = 0; f < g.length; f++) {
          if (b.multiVal.indexOf(g[f]) == -1) {
            ...
            Util.globalTopTip('已存在，请勿重复添加', 'top_error', '3000', $('#colla_add'), true);
          }
        }
        return e;
      },
  ...
  collaInviteTip: function (b) {
    if (!$('#colla_invite_tip').length) {
      ...
      c += '</ul><div style="text-align:right"><span class="send pro-btn default okbtn">我知道了</span></div></div>';
      $(c).appendTo('body');
    }
    ...
    $('#colla_invite_tip .okbtn')
      .off()
      .click(function () {
        $('#colla_invite_tip .mind-dlog-close,#colla_invite_tip .dialog-close').click();
      });
  },
  multiType: [],
  multiVal: [],
  ...
  showCollaList: function (d) {
    ...
    for (var b = 0; b < e.length; b++) {
      var a = e[b];
      c +=
        '<div><span class="pop-text">' + a.fullName + '</span><span class="pop-text-i"><' + a.email + '></span></div>';
    }
    return c;
  },
    ...
    if (e != null && e.contacters.length > 0) {
      for (var c = 0, a = e.contacters.length; c < a; c++) {
        ...
        d +=
          ...
          '</span><span>' +
          (typeof pageContext != 'undefined' && pageContext.nickNameData && pageContext.nickNameData[b.userId]
  ...
  showFollowinger: function (d, g, j) {
    ...
    if (d != null && a > 0) {
      for (var e = 0, h = a; e < h; e++) {
        ...
        var b = '<div class="user-logo tmu-photo">' + collaboration.loadAvatar(c.userId) + '</div>';
        f +=
          '<div uid="' +
          c.userId +
          ...
          b +
          '</span><span>' +
          (typeof pageContext != 'undefined' && pageContext.nickNameData && pageContext.nickNameData[c.userId]
    ...
    if (g.find('.colla-user ').length < 1) {
      ...
      g.html(
        "<div class='colla-users-none'><img src='/assets/images/icon/empty_contact.svg'/><div>" + l + '</div></div>',
      );
    }
  },
  showOrgs: function (e) {
  ...
  getRoleList: function (e, d) {
    ...
    if (typeof chartId == 'undefined') {
      ...
      f = { folderId: a == 'folder' ? g : '', chartId: a == 'chart' ? g : '', pg: d || 1 };
    } else {
  ...
  showRoleList: function (g) {
    ...
    if (g.owner != null) {
      e =
        ...
        (typeof pageContext != 'undefined' && pageContext.nickNameData && pageContext.nickNameData[g.owner.userId]
    ...
    if (g.collaborations != null) {
      for (var d = 0; d < g.collaborations.length; d++) {
        ...
        var f = '<span class="item-role-status" >' + (c.email ? c.email : c.phone) + '</span>';
        if (c.email && c.phone) {
          f =
        ...
        var b = '<div class="user-logo tmu-photo">' + collaboration.loadAvatar(a.userId) + '</div>';
        e +=
          '<div id="item_' +
          c.collborationId +
          ...
          b +
          '</span><span class="item-user-fullname">' +
          (typeof pageContext != 'undefined' && pageContext.nickNameData && pageContext.nickNameData[a.userId]
