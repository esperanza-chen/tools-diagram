  ...
  if (document.getElementById('support_canvas').getContext) {
    c(function (m) {
      if (m.page.backgroundColor) {
        $('#view_container').css('background-color', 'rgb(' + m.page.backgroundColor + ')');
      }
      Designer.open(m);
...
function initChartOperate() {
  ...
  $('.mind-view-left').css({ width: $('.mind-view-left').innerWidth(), left: 0 });
  $('.temp .info-temp')
    .off()
    .on('click', '.temp-list-item', function () {
      ...
      if ($(this).parent().hasClass('works-content')) {
        poCollect('详情-点击', { 名称: '详情-点击用户其他作品' });
        window.location.href = k;
      } else {
        poCollect('详情-点击', { 名称: '详情-点击猜你喜欢' });
        window.open(k);
      }
    });
  $('.item[share]').on('click', function (l) {
    ...
    var i = $('.pop-con.share'),
      k = $(this);
    if (i.css('opacity') == 1) {
      i.css({ opacity: 0, right: -446 });
      k.removeClass('active');
    } else {
      i.css({ opacity: 1, display: 'block', right: '2px' });
      k.addClass('active');
    }
    zhuge.track('点击分享到社交网络', { 触发位置: '文件预览' });
  });
  $('body').on('click', function () {
    var i = $('.pop-con');
    i.css({ opacity: 0, display: 'none', right: -446 });
  ...
  $('#userReport').on('click', function () {
    ...
    $('#btn_submit_report')
      ...
      .on('click', function () {
        ...
        if (!k) {
          Util.globalTopTip('举报理由不能为空', 'top_error', 3000, $('#report-page'), true);
          return;
        } else {
          if (k.length > 200) {
        ...
        Util.ajax({
          url: '/view/report',
          data: { chartId: chartId, content: k, category: i },
          success: function (l) {
  ...
  $('#view_container')
    ...
    .on('touchstart', function (n) {
      ...
      var k = $('#designer_canvas').position();
      $('#view_container').css('cursor', 'move');
      var i = { disabled: true, scale: 1, distance: 0, centerX: 0, centerY: 0 };
      if (m.length == 2) {
        i = l(m);
      }
      $('#view_container')
        ...
        .on('touchmove', function (u) {
          u.preventDefault();
          var q = u.originalEvent.touches;
          if (!i.disabled) {
            var v = l(q);
            var t = (v.distance - i.distance) / 150;
            var s = Math.round((i.scale + t) * 100) / 100;
            Designer.setZoomScale(s, { x: i.centerX, y: i.centerY });
          } else {
            var w = m[0];
            var o = q[0];
            var r = o.pageX - w.pageX;
            var p = o.pageY - w.pageY;
            $('#designer_canvas').css({ left: k.left + r + 'px', top: k.top + p + 'px' });
          }
        });
      $('#view_container')
        .off('touchend')
      ...
      function l(q) {
        ...
        return {
          disabled: false,
          scale: Designer.config.scale,
          distance: Math.sqrt(Math.pow(p - s, 2) + Math.pow(o - r, 2)),
          centerX: (p + s) / 2,
          centerY: (o + r) / 2,
        };
      }
  ...
  var f = c.requestFullScreen || c.webkitRequestFullScreen || c.mozRequestFullScreen || c.msRequestFullScreen;
  var b =
  ...
  $('#view_container').on('mousewheel DOMMouseScroll', function (l) {
    var k = l.originalEvent.wheelDelta || -l.originalEvent.detail;
    var m = Math.max(-1, Math.min(1, k));
    if (l.ctrlKey || l.metaKey) {
      l.preventDefault();
      var i = { x: l.originalEvent.pageX, y: l.originalEvent.pageY };
      if (m < 0) {
        Designer.zoomOut(i);
      } else {
        Designer.zoomIn(i);
  ...
  $('#view_container').css('backgroundColor', $('#designer_canvas').css('backgroundColor'));
  $('#view_container').mousedown(function (k) {
    ...
    $(document).bind('mousemove.drag', function (p) {
      var o = p.pageX - n;
      var q = p.pageY - i;
      $('#designer_canvas').css({ left: m + o + 'px', top: l + q + 'px' });
    });
  ...
  $('.item[info]')
    ...
    .on('click', function () {
      ...
      if (!$(this).hasClass('active') && $('.item[comment]').hasClass('active')) {
  ...
  if (!localStorage.getItem('infoState') || localStorage.getItem('infoState') == 'open') {
    setTimeout(function () {
      $('.item[info]').trigger('click');
    }, 800);
  }
...
function hintTip(c, a, e, d) {
  ...
  if (b.length == 0) {
    b = $("<div id='mind-tip'><div class='mind-tip-text'></div></div>").appendTo('body');
  ...
  b.css({ marginLeft: -b.width() / 2 - 30 });
  if (e != null && e == 'left_bottom') {
    b.show().css({ bottom: 100, left: 25, marginLeft: '0', top: 'initial' });
  } else {
    b.show().css({ top: 60 });
  }
...
function templates() {
  $('.info-temp .chart-img').click(function () {
    poCollect('详情-点击', { 名称: '详情-点击头像' });
  });
  $.ajax({
    url: '/templates/author_templates',
    data: { page: 1, pageSize: 5, ownerId: ownerId },
    success: function (b) {
      var a = b.rows;
      if (a && a.length > 0) {
        renderWorks(a);
  ...
  $.ajax({
    ...
    success: function (b) {
      ...
      if (a && a.length > 0) {
        if (
          (a[0].specialIds && a[0].specialIds.length > 0 && a[0].publicClonePrice > 0) ||
...
function increaseZoomScale(h) {
  ...
  var a = { x: b.width() / 2, y: b.height() / 2 };
  var g = a.x - c.position().left;
  var e = a.y - c.position().top;
  var f = g * (1 + h);
  var d = e * (1 + h);
  c.css({ left: a.x - f, top: a.y - d });
  zoom += h;
  Designer.setZoomScale(zoom);
}
...
function fullDocument() {
  ...
  $('#view_container').css({ height: '100vh' });
...
function exitFullDocument() {
  ...
  $('#view_container').css({ height: $(window).height() - a });
...
function newCreateConfirm(d, c, a, b) {
  ...
  Util.ajax({
    ...
    data: { chartId: d, teamId: c, orgId: a },
    success: function (f) {
      Util.loadingball({ close: true });
      if (f.result == 'clone') {
        poCollect('模版-克隆成功');
        location =
      ...
      if (f.result == 'cloneBuy') {
        poCollect('点击触发-支付弹窗', { 渠道来源: b ? '猜你喜欢' : '预览页' });
        Util.payWindow('open', { chartId: d, price: f.price }, function () { });
        return;
      }
      if (f.result == 'overd') {
        Util.globalTopTip(
        ...
        if (f.result == 'chart_error') {
          Util.globalTopTip('很抱歉，该模板已被作者取消公开或删除。请继续浏览并使用其他模板～', 'top_error', 5000);
          return;
        }
      }
...
function submitComment(d, c, f, e) {
  ...
  if (b == '&lt;br&gt;' || b == '') {
    ...
    Util.ajax({
      ...
      data: { chartId: chartId, content: b, ownerId: ownerId, refId: c, parentId: f },
      success: function (i) {
        if (i.result == 'error_text') {
          Util.globalTopTip('描述或标签中存在敏感词汇，请修改后再发布', 'top_error', 3000, $('.controlls'), true);
          return;
        }
        poCollect('模版详情页-评论', { 名称: '评论-发表' });
        var g = $('#comment_count'),
          h = parseInt(g.text()) + 1;
        g.text(h);
...
function commentRef(c, b, f) {
  ...
  if (e.length == 0) {
    ...
    $(c).parent().after(e);
    $(c).parent().next('.reply_box_ref').attr({ id: b, pId: f });
    function d() {
    ...
    e.children('.txt')
      ...
      .streamInput({ face: $('#showfaces-reply') });
    var a = e.find('.button');
    a.disable();
...
function openDeldia(c, a, b) {
  $.confirm({
    ...
    onConfirm: function () {
      $(b).disable();
      Util.get('/view/delComments', { refId: a, chartId: chartId, parentId: c }, function (f) {
        var d = '';
        if ($(b).parents('div').hasClass('reply_list-item')) {
          d = 1;
          $(b).parents('.reply_list-item').remove();
        } else {
          d = $(b).parents('li').find('li').length + $(b).parents('li').find('.reply_list-item').length + 1;
          $(b).parents('li').remove();
        }
        var g = $('#comment_count'),
          e = Number(g.text()) - d;
...
function showFaces(c, b) {
  var a = $('#faces_dialog'),
    d = b || $('#comment');
  if (a.length > 0) {
    a.popMenu({ target: c, position: 'right', zindex: 19 });
    return;
  }
  var a = $(
  ...
  a.appendTo('body');
  a.popMenu({ target: c, position: 'right' });
...
function loadComments() {
  ...
  Util.ajax({
    ...
    data: { chartId: chartId, skip: currentSkip, ownerId: ownerId },
    success: function (c) {
      $('.reply_list').append(c);
...
function doLikeChart() {
  ...
  if (b.hasClass('active')) {
    ...
    poCollect('详情-点击', { 名称: '详情-模板点赞' });
    b.numberTip();
    b.find('.ico_box').css({ 'vertical-align': 'middle', 'margin-top': '-4px', width: '16px' });
    $('#like-count').css({ 'vertical-align': 'middle', 'margin-top': '2px' });
    b.find('.icons').hide();
    ...
    success: function (c) {
      ...
      if (c.result) {
        ...
        b.find('.ico_box')
          ...
          .css({ 'vertical-align': 'unset', 'margin-top': '0' });
        $('#like-count').css({ 'vertical-align': 'unset', 'margin-top': '0' });
        b.attr('class', 'item');
        $(b).find('.ico_box .icons').html('&#xe6d1;');
      }
...
function showWeixin(g) {
  ...
  $('#pageurl').attr('src', b);
  $('#pageurl_div').css({ right: '220px', top: '60px' }).show();
...
function renderTemp(e) {
    ...
    if (e[b].chartId != chartId) {
      ...
      if (d > 0) {
        for (j = 0; j < d; j++) {
          ...
          if (e[b].specialIds && e[b].specialIds.indexOf(activityList[j].specialId) > -1) {
            a = true;
            e[b]['publicClonePrice'] = activityList[j].price;
          }
        }
      }
      var f = imagePath + '/chart_image/thumb/' + e[b].thumbnail + '.png?cid=' + e[b].chartId;
      if (e[b].category.indexOf('mind') >= 0) {
        f = imagePath + '/chart_image/thumb/mind/' + e[b].thumbnail + '.png?cid=' + e[b].chartId;
      } else {
        if (e[b].category.indexOf('board') >= 0) {
          f = "https://assets.processon.com/chart_image/thumb/'+chart.thumbnail+'.png";
        }
      }
      c +=
        '<li class="temp-list-item" chartid="' +
        ...
        (e[b].talent ? "<img class='talent-lab' src='/assets/images/new/talent_lab.svg'>" : '') +
        "<img src='" +
        (e[b].owner ? '/photo/' + e[b].owner.userId + '.png' : '/assets/imgs/on.png') +
  ...
  $('.related-temp .clone-btn').click(function (h) {
    ...
    newCreateConfirm(g.attr('chartId'), g.attr('teamId'), g.attr('orgId'), true);
  });
}
...
var follow = {
  ...
  unFollow: function (b, a, c) {
    ...
    $.confirm({
      ...
      onConfirm: function () {
        $(b).disable();
        Util.get('/start/unfollow', { followId: a }, function (d) {
          $(b).enable();
          if (d.result == 'success') {
  ...
  saveFollow: function (b, a, c) {
    ...
    this.following = true;
    $(b).disable();
    Util.get('/start/savefollow', { followIds: a }, function (d) {
      $(b).enable();
      if (d.result == 'success') {
