var TeamShape = {
  ...
  init: function () {
    ...
    $('#shape_panel')
      ...
      .on('click', '.categories_edit li', function (c) {
        ...
        if (b.attr('tit') == 'delete') {
          ...
          var d = f.text();
          f.html('<input type="text" class="panel_team_title_input" value="' + d + '"/>');
    ...
    $('#shape_panel')
      ...
      .on('click', '.team_shape_edit li', function (k) {
        ...
        if (l == 'edit') {
          var f = { elements: {} };
          TeamShape.shapeMaps[b].elements = TeamShape.utils.resetShapes(TeamShape.shapeMaps[b].elements, {
            x: 100,
            y: 100,
          });
          ...
          $('.team_shape_title').text('编辑组件: ' + TeamShape.shapeMaps[b].title);
          $('#team_shape_form_title').val(TeamShape.shapeMaps[b].title);
          $('#team_shape_form_def').val(window.btoa(encodeURIComponent(JSON.stringify(f))));
          ...
          if (l == 'copy') {
            ...
            if (l == 'move') {
              ...
              if (l == 'info') {
                ...
                var n = g.text();
                g.html('<input type="text" class="panel_team_shape_input" value="' + n + '"/>');
  ...
  initEvt: function () {
    ...
    $('.panel_team_container')
      ...
      .on('mouseup.myshapes', function () {
        ...
        if (isComponentAdmin != 'true') {
          Util.globalTopTip('您当前没有操作权限，请联系管理员开通', 'top_error', 2000, $('#designer'), true);
          return;
        }
        var c = $(this).attr('id');
        var p = Utils.getSelectedIds();
        if (p.length > 100) {
          Util.globalTopTip('图形数量不能超出100', 'top_error', 2000, $('#designer'), true);
          return;
        }
          ...
          $.ajax({
            ...
            success: function (i) {
              if (i.code == '200') {
                ...
                Util.globalTopTip(e.message, 'top_error', 2000, $('#designer_viewport'), true);
              }
            },
            error: function () {},
          });
      ...
      .on('mousedown', function (s) {
        ...
        $('#canvas_container').bind('mousemove.create', function (A) {
          ...
          for (var x = 0; x < n.length; x++) {
            ...
            if (v.name == 'linker') {
              v.from = $.extend(true, v.from, TeamShape.utils.resetPosByMouse(D, v.from_, v));
              v.to = $.extend(true, v.to, TeamShape.utils.resetPosByMouse(D, v.to_, v));
              if (v.linkerType == 'broken' || v.linkerType == 'curve') {
                for (var y = 0; y < v.points_.length; y++) {
                  v.points[y] = TeamShape.utils.resetPosByMouse(D, v.points_[y], v);
                }
              }
            }
            var u = v.props;
            ...
            C.css({
              left: v.props.x.toScale() - (C.width() - v.props.w.toScale()) / 2 + 'px',
              top: v.props.y.toScale() - (C.height() - v.props.h.toScale()) / 2 + 'px',
    ...
    $('.edit_team_container')
      ...
      .on('click', function (g) {
        ...
        $('#shape_panel .categories_edit').css({ left: $(this).offset().left + 'px' });
      });
    $('.edit_team_shape')
      .off()
      .on('click', function (g) {
        ...
        $('#shape_panel .team_shape_edit').css({ left: $(this).offset().left + 'px' });
      });
    $('.panel_team_box')
      .off('mouseenter')
      .on('mouseenter', function () {
        ...
        f.css({ width: d.imgW, height: d.imgH, transform: 'scale(' + 200 / d.imgH + ')' });
        f.attr('src', d.thumbnail);
        c.attr('current', j);
        c.show();
        var i = b.offset().top - $('#designer_header').outerHeight() + b.height() / 2 - c.outerHeight() / 2;
        if (i < 5) {
          i = 5;
        } else {
  ...
  onmessage: function (c) {
    ...
    TeamShape.updataShape(TeamShape.acComponentId, TeamShape.acCategoryId, JSON.stringify(a), b);
  ...
  renderPanelItem: function (h, k) {
    var g = this;
    var m = this.config.panelItemWidth,
      f = this.config.panelItemHeight;
    var l = $('#teamshape_' + h.id + '');
    var i = false;
    if (l.length == 0) {
      var d =
        ...
        "' shapeId ='" +
        h.id +
        "'><div style='width: 70px;height: 70px;position: absolute; z-index: 1;'></div> <span class='panel_team_item_bg'> </span><p class='panel_team_item_title'>" +
        h.title +
        '</p> ' +
      ...
      i = true;
      if (k) {
        panelBox = $(d).appendTo('#panel_team_' + h.category);
      } else {
        panelBox = $(d).prependTo('#panel_team_' + h.category);
      }
    }
    var j = 60,
      b = 60,
    ...
    c.onload = function () {
      ...
      if (i) {
        $('#teamshape_' + h.id + '')
          .children('.panel_team_item_bg')
          .html(
            "<iframe class='panel_team_item' scrolling='no' style='border:none; transform: scale(" +
              a +
              ");' src='" +
              h.thumbnail +
              "' width='" +
        ...
        l.children('img').attr('src', h.thumbnail);
      }
      l.children('img').css({});
      g.initEvt();
      c = null;
  ...
  removeShape: function (d, b, c) {
    ...
    $.confirm({
      content:
        "是否要删除图形 <span style='color:#4386F5'>「" + c + '」</span> ，请确认？<br/> 此操作无法恢复，请谨慎操作。',
      okval: '删除',
      ...
      onConfirm: function () {
        $.ajax({
          ...
          success: function (f) {
            if (f.code == '200') {
              Util.globalTopTip('组件删除成功', 'top_success', 2000, $('#designer_viewport'), true);
              a.getCategoryList(b);
              delete a.shapeMaps[d];
            } else {
              Util.globalTopTip(f.message, 'top_error', 2000, $('#designer_viewport'), true);
            }
          },
          error: function () {
            Util.globalTopTip('组件删除失败', 'top_error', 2000, $('#designer_viewport'), true);
          },
        });
      },
    });
  ...
  copyShape: function (a, c) {
    ...
    $.ajax({
      ...
      success: function (d) {
        if (d.code == '200') {
          Util.globalTopTip('复制图形成功', 'top_success', 2000, $('#designer_viewport'), true);
          b.getCategoryList(c);
        } else {
          Util.globalTopTip(d.message, 'top_error', 2000, $('#designer_viewport'), true);
        }
      },
      error: function () {
        Util.globalTopTip('复制图形失败', 'top_error', 2000, $('#designer_viewport'), true);
      },
    });
  },
        ...
        $('.team-category-footer .okbtn')
          ...
          .on('click', function () {
            if (TeamShape.moveCategorieId == '') {
              Util.globalTopTip('请选择要移动的项目组', 'top_error', 2000, $('#teamcategory_dialog'), true);
              return;
            }
            if (TeamShape.moveCategorieId == c) {
              Util.globalTopTip('图形已在分类中', 'top_error', 2000, $('#teamcategory_dialog'), true);
              return;
            }
            $.ajax({
              url: b.config.host + '/api/chart/component/move',
              type: 'post',
              data: { componentId: a, toComponentItemId: TeamShape.moveCategorieId },
              success: function (i) {
                if (i.code == '200') {
                  Util.globalTopTip('移动图形成功', 'top_success', 2000, $('#teamcategory_dialog'), true);
                  b.getCategoryList(TeamShape.moveCategorieId);
                  b.getCategoryList(c);
                  TeamShape.moveCategorieId = '';
                  $('#teamcategory_dialog').dialog('close');
                } else {
                  Util.globalTopTip(i.message, 'top_error', 2000, $('#designer_viewport'), true);
                }
              },
              error: function () {
                Util.globalTopTip('移动图形失败', 'top_error', 2000, $('#teamcategory_dialog'), true);
              },
            });
            $('#teamCategory_dialog').dialog('close');
          });
  ...
  addCategory: function () {
    ...
    $('.add-category-footer .okbtn')
      ...
      .on('click', function () {
        ...
        if (b == '') {
          Util.globalTopTip('文件夹名不能为空', 'top_error', 2000, $('#addcategory_dialog'), true);
          return;
        }
        $.ajax({
          url: a.config.host + '/api/chart/component/item/save',
          type: 'post',
          data: { orgId: orgId, title: b, ownerId: userId, ownerName: userName },
          success: function (c) {
            if (c.code == '200') {
  ...
  removeCategory: function (b, c) {
    ...
    $.confirm({
      content:
        "是否要删除组件 <span style='color:#4386F5'>「" + c + '」</span> ，请确认？<br/> 此操作无法恢复，请谨慎操作。',
      okval: '删除',
      tokenClass: 'team-shape-remove-confirm',
      onConfirm: function () {
        $.ajax({
          ...
          success: function (d) {
            if (d.code == '200') {
              Util.globalTopTip('删除分类成功', 'top_success', 2000, $('#designer_viewport'), true);
              $('#' + b).remove();
            } else {
              Util.globalTopTip(d.message, 'top_error', 2000, $('#designer_viewport'), true);
            }
          ...
          error: function () {
            Util.globalTopTip('删除分类失败', 'top_error', 2000, $('#designer_viewport'), true);
          },
        });
      },
    });
  ...
  updataCategory: function (b, c) {
    ...
    $.ajax({
      ...
      success: function (d) {
        if (d.code == '200') {
          Util.globalTopTip('更换标题成功', 'top_success', 2000, $('#designer_viewport'), true);
      ...
      error: function () {
        Util.globalTopTip('更换标题失败', 'top_error', 2000, $('#designer_viewport'), true);
      },
    });
  },
  ...
  updataShape: function (b, f, d, a) {
    ...
    $.ajax({
      ...
      success: function (g) {
        if (g.code == '200') {
          Util.globalTopTip('编辑组件成功', 'top_success', 2000, $('#designer_viewport'), true);
          c.getCategoryList(f);
        }
      },
      error: function () {
        Util.globalTopTip('编辑组件失败', 'top_error', 2000, $('#designer_viewport'), true);
      },
    });
  },
  ui: {
    init: function () {
      ...
      $('.goto_team_container')
        ...
        .on('click', function (b) {
          b.stopPropagation();
          window.open('/corp/manage/index?orgId=' + orgId + '#category-management');
        });
      $('#shape_panel .panel_team_add').css('display', 'none');
      $('#shape_panel .panel_container').each(function (b, c) {
        if (b > 0) {
  ...
  utils: {
    ...
    getCreatingImg: function (d) {
      ...
      if (c.length == 0) {
        a = $("<div id='creating_shape_container'></div>").appendTo('#designer');
        c = $(
      ...
      a.css({ left: '0px', top: '0px', width: $('.panel_container').width(), height: $('#shape_panel').outerHeight() });
      return c;
    },
    setCreatingImg: function (b, c) {
      $('#creating_shape_container').show();
      var a = Utils.getRelativePos(c.pageX, c.pageY, $('#creating_shape_container'));
      b.css({
  ...
  flow2svg: {
    init: function (b) {
      var m = this;
      var r = m.calcCanvasSize(b);
      var n = Model.define.theme == null ? false : true;
      var a = Model.define.elements;
      if ($('#team_thumbnail_box').length == 0) {
        $('body').append("<div id='team_thumbnail_box' style='display:none;'></div>");
      }
      var s = r.w > r.h ? r.w : r.h;
      var q = SVG('team_thumbnail_box').size(s, s);
      b = b.sort(function (j, i) {
        return j.props.zindex - i.props.zindex;
      });
      for (var h = 0, l = b.length; h < l; h++) {
        var o = Utils.copy(b[h]);
        if (o.name == 'linker') {
          flow2svg.linker2svg(o, q, r, n);
        } else {
          if (o.parent == '' || o.parent == null) {
            flow2svg.shape2svg(o, q, r);
            if (o.children != null && o.children.length > 0) {
              var d = o.children;
              for (var g = 0; g < d.length; g++) {
                var f = d[g];
                var c = Utils.copy(a[f]);
                flow2svg.shape2svg(c, q, r, n);
              }
            }
          }
        }
      }
      q.attr('viewBox', '0 0 ' + r.w + ' ' + r.h);
      var p = $('#team_thumbnail_box')
        .html()
        .replace(/&nbsp;/g, '\u00a0');
        console.log(p)
      $('#team_thumbnail_box').remove();
      var k = 'data:image/svg+xml;base64,' + window.btoa(unescape(encodeURIComponent(p)));
      return k;
    },
