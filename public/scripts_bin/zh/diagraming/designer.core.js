...
window.Model = {
  ...
  create: function (e, b, h) {
    ...
    var g = Model.define;
    if (g.defaultStyle != null && g.defaultStyle.fillStyle != null && $.isEmptyObject(c.fillStyle)) {
      c.fillStyle = g.defaultStyle.fillStyle;
    }
    if (g.defaultStyle != null && g.defaultStyle.lineStyle != null && $.isEmptyObject(c.lineStyle)) {
      c.lineStyle = g.defaultStyle.lineStyle;
    }
    if (g.defaultStyle != null && g.defaultStyle.fontStyle != null && $.isEmptyObject(c.fontStyle)) {
  ...
  createByShape: function (c, b, g) {
    ...
    var f = Model.define;
    if (f.defaultStyle != null && f.defaultStyle.fillStyle != null && $.isEmptyObject(d.fillStyle)) {
      d.fillStyle = f.defaultStyle.fillStyle;
    }
    if (f.defaultStyle != null && f.defaultStyle.lineStyle != null && $.isEmptyObject(d.lineStyle)) {
      d.lineStyle = f.defaultStyle.lineStyle;
    }
    if (f.defaultStyle != null && f.defaultStyle.fontStyle != null && $.isEmptyObject(d.fontStyle)) {
  ...
  removeById: function (id) {
    let shape = Model.getShapeById(id)
    Model.remove()
  },
  remove: function (b, k) {
  ...
  clearDefaultStyle: function () {
    ...
    if (Model.define.defaultStyle == null && Model.define.defaultLinkerStyle == null) {
      return;
    }
  ...
  getOrderListGroup: function (a, b) {
    return this.orderListGroup[Math.ceil(a.restoreScale() / 500) + '_' + Math.ceil(b.restoreScale() / 500)] || [];
  },
...
window.Designer = {
  ...
  initialize: {
    ...
    initCanvas: function () {
      ...
      const res = Object.values(dataJSON.elements);
      if (res.length > 0) {
        let maxWidth = res[0].props.w + res[0].props.x;
        let maxHeight = res[0].props.h + res[0].props.y;
        for (let i = 1; i < res.length; i++) {
          if (maxWidth < (res[i].props.w + res[i].props.x))
            maxWidth = res[i].props.w + res[i].props.x;
          if (maxHeight < (res[i].props.h + res[i].props.y))
            maxHeight = res[i].props.h + res[i].props.y;
        }
        maxWidth = maxWidth > 1340 ? maxWidth : 1340;
        maxHeight = maxHeight > 800 ? maxHeight : 800;
        Model.define.page.width = maxWidth;
        Model.define.page.height = maxHeight;
      }
      var k = Model.define.page.width.toScale();
      var r = Model.define.page.height.toScale();
      if (k > 20000) {
        k = 20000;
      }
      if (r > 20000) {
      ...
      var t = Model.define.page.backgroundColor;
      if (t == 'transparent') {
        t = '225,234,247';
      }
      var c = Utils.getDarkerColor(t);
      var q = Utils.getDarkestColor(t);
      $('#designer_canvas').css({
      ...
      if (f.length > 0) {
        ...
        p.clearRect(0, 0, k, r);
        var n = Model.define.page.padding.toScale();
        var d = k - n * 2;
        var l = r - n * 2;
        ...
        p.rect(n, n, d, l);
        p.fill();
        var j = Math.round(Model.define.page.gridSize.toScale());
        if (j < 10) {
          j = 10;
        }
        ...
        //   p.translate(-n, -n);
        // }
        if (Model.define.page.showPrintLine) {
          p.lineWidth = 2;
          p.strokeStyle = 'rgb(' + Utils.getDarkerColor(t, 40) + ')';
          p.setLineDash([5]);
          var m = (1485).toScale(),
            e = (1050).toScale();
          if (Model.define.page.orientation == 'landscape') {
        ...
        if (Model.define.page.watermark === '' || typeof Model.define.page.watermark === 'undefined') {
        ...
        if ($('#designer_watermark_canvas').length <= 0) {
          $('#designer_canvas').append(
            '<canvas id="designer_watermark_canvas"  width="' + a + '" height="' + b + '"></canvas>',
          );
        ...
        var p = $('#designer_watermark_canvas')[0].getContext('2d');
        p.beginPath();
        p.clearRect(0, 0, a, b);
        p.rect(0, 0, a, b);
        p.fill();
      ...
      function g() {
        var v = Model.define.page.watermark;
        if (v) {
          v = $('<span/>').html(v).text();
          if (!$('#watermarkCanvas').length) {
            var x = '<canvas id = "watermarkCanvas" width = "300px"  height = "300px" style="display:none;"></canvas>';
            $('body').append(x);
          }
          var u = $('#watermarkCanvas')[0];
          var w = u.getContext('2d');
          w.clearRect(0, 0, 300, 300);
          w.font = '18px Arial';
          w.rotate((-45 * Math.PI) / 180);
          w.fillStyle = 'rgba(' + q + ',0.8)';
          w.fillText(v, -190, 210);
          w.rotate((45 * Math.PI) / 180);
          var h = p.createPattern(u, 'repeat');
          p.translate(n, n);
          p.rect(0, 0, d, l);
          p.fillStyle = h;
          p.fill();
    ...
    initShapes: function () {
      ...
      // console.log(Schema.categories,"dgshafdasgd")
      let schemaTemp = JSON.parse(JSON.stringify(Schema.categories));
      Schema.categories[0] = schemaTemp[2];
      Schema.categories[1] = schemaTemp[3];
      Schema.categories[2] = schemaTemp[1];
      Schema.categories[3] = schemaTemp[0];
      Schema.categories[2].text = "形状组件"
      // console.log(Schema.categories,"dgshafdasgd")
      ...
      if (indexs > 350) {
        Utils.toast('当前加载的图形过多，可能会造成页面卡顿，<br><br>请取消勾选不需要的图形', '.designer_button', 9000);
      }
      ...
      function appendPanelItem(shape, group, addShape) {
        ...
        // console.log(shape, 'kkkkkkkkkkkk')
        var html =
        ...
        if (group) {
          panelBox.append(
            "<div class='group_icon' onmousedown='Designer.op.showPanelGroup(\"" + group + '", event, this)\'></div>',
          );
        }
        ...
        panelBox
          .bind('mouseenter', function () {
            ...
            thumb.children('div').html(shape.title), (canvas = thumb.children('canvas')[0]);
            ...
            if (shape.category == 'myshape') {
              ...
              if (shape.boxPos.w >= shape.boxPos.h) {
                if (shape.boxPos.w > maxWidth) {
                  ...
                  boxPos.w = maxWidth;
                  boxPos.h = parseInt((shape.boxPos.h / shape.boxPos.w) * boxPos.w);
                }
              } else {
                if (shape.boxPos.h > maxHeight) {
                  scale = maxHeight / shape.boxPos.h;
                  boxPos.h = maxHeight;
                  boxPos.w = parseInt((shape.boxPos.w / shape.boxPos.h) * boxPos.h);
                }
              }
              ...
              for (var i = 0; i < myshape.elements.length; i++) {
                ...
                if (shapeItem.name == 'linker') {
                  ...
                  shapeItem.lineStyle.lineWidth = (lineStyle.lineWidth * scale) / 2;
                } else {
              ...
              var props = {
                ...
                w: shape.props.w,
                h: shape.props.h,
                angle: shape.props && shape.props.angle
              };
            ...
            if (top < 5) {
              ...
              if (top + thumb.outerHeight() > $('#designer_viewport').height() - 5) {
          ...
          .live('mousedown', function (downE) {
            ...
            $('#canvas_container').bind('mousemove.create', function (e) {
              ...
              if (createdShapes == null) {
                // console.log(e,name, 5555)
                createdShapes = createShape(name, location.x, location.y);
                // createdShapes = arrStr
                // console.log(createdShapes, 999888)
              ...
              // console.log(createdBoxs,"bababab4545")
              for (var j = 0; j < createdShapes.length; j++) {
                var createdBox = createdBoxs[j];
                var createdShape = createdShapes[j];
                if (createdShape.boxPos) {
                  createdShape.props.x = location.x.restoreScale() - createdShape.boxPos.w / 2 + createdShape.pos.x;
                  createdShape.props.y = location.y.restoreScale() - createdShape.boxPos.h / 2 + createdShape.pos.y;
                } else {
                ...
                if (createdShape.name == 'linker') {
                  ...
                  if (createdShape.linkerType == 'broken' || createdShape.linkerType == 'curve') {
                    for (var i = 0; i < createdShape.points_.length; i++) {
                      createdShape.points[i] = resetPosByMouse(location, createdShape.points_[i], createdShape);
                    }
                  }
                }
                var p = createdShape.props;
                var snaped = Designer.op.snapLine(p, [createdShape.id], true, createdShape, shapeBoxs);
                if (snaped.attach) {
                ...
                createdBox.css({
                  left: createdShape.props.x.toScale() - (createdBox.width() - createdShape.props.w.toScale()) / 2 + 'px',
                  top: createdShape.props.y.toScale() - (createdBox.height() - createdShape.props.h.toScale()) / 2 + 'px',
            ...
            $(document).bind('mouseup.create', function () {
              ...
              if (createdShapes != null) {
                if (created == false) {
                  ...
                  for (var i = 0; i < anchorInLinkers.length; i++) {
                    ...
                    if (anchorInLinker.type == 'line') {
                      ...
                      if (anchorInLinker.anchors.length == 1) {
                        ...
                        var angle = Utils.getPointAngle(createdShapes[0].id, anchor.x, anchor.y, radius);
                        linker.to = {
                        ...
                        if (anchorInLinker.anchors.length == 2) {
                          ...
                          var angle = Utils.getPointAngle(createdShapes[0].id, toAnchor.x, toAnchor.y, radius);
                          linker.to = {
                            id: createdShapes[0].id,
                            x: toAnchor.x,
                            y: toAnchor.y,
                          ...
                          angle = Utils.getPointAngle(createdShapes[0].id, fromAnchor.x, fromAnchor.y, radius);
                          newLinker.from = {
                            id: createdShapes[0].id,
                            x: fromAnchor.x,
                            y: fromAnchor.y,
                      ...
                      if (anchorInLinker.anchors.length <= 2) {
                        ...
                        newLinker = Designer.op.beautifyLinkerFocus(newLinker, true);
                  ...
                  Designer.events.push('resetBrokenLinker');
                  try {
                    _czc.push(['_trackEvent', 'flow', 'create_shape_panel', createdShapes[0].name]);
                  } catch (e) {}
                }
              }
              currentShape.css({
                ...
                $.getJSON(
                  ...
                  function (data) {
                    ...
                    if (data.elementsCount == 0) {
                      $('.search_panel_loading').text('没有搜索到网络图标，建议更换关键词再次尝试。');
                      return;
                    }
                    if (searchOffset == 1) {
                      $('#panel_search .panel_box').each(function (index, item) {
                        if ($(item).attr('shapename').indexOf('image_search_') > -1) {
                          $(item).remove();
                        }
                      });
                    ...
                    if (searchOffset < data.elementsCount) {
                      var btn = $("<div class='toolbar_button active search_panel_btn'>加载更多</div>").appendTo(
                        '#panel_search',
                      );
                      btn.bind('click', function () {
                        searchIconfinder();
      ...
      $('.panel_myshape_container').on('mouseup.myshapes', function () {
        ...
        if (selectedIds.length > 50) {
          Util.globalTopTip('最多可以包含50个图形组合', 'top_error', 2000, $('#designer'), true);
          return;
        }
        ...
        if (selectedIds.length > 0) {
            ...
            if (shape.children) {
              for (var j = 0; j < shape.children.length; j++) {
                if (selectedIds.indexOf(shape.children[j]) < 0) {
                  var childrenShape = Utils.copy(Model.getShapeById(shape.children[j]));
                  shapes.push(childrenShape);
                }
              }
            }
          }
          var containedShapes = Utils.copyArray(Utils.getContainedShapes(shapes));
          ...
          for (key in def) {
            if (def[key].name == 'linker') {
              ...
              if (containedShapeIds.indexOf(cLinker.to.id) > -1 && containedShapeIds.indexOf(cLinker.from.id) > -1) {
                shapes.push(cLinker);
              }
            }
          }
          ...
          for (var i = 0; i < shapes.length; i++) {
            ...
            if (shapeItem.name == 'linker') {
              ...
              if (shapeItem.linkerType == 'broken' || shapeItem.linkerType == 'curve') {
          ...
          if (Schema.myShapes.length == 9) {
            Designer.events.push('isMember', function (isMember) {
              if (isMember) {
                ...
                Util.globalTopTip(
                  "升级到个人版或者团队版后，才可以继续添加图形， <a style='color:#fff;letter-spacing:1px;' href='/upgrade?pos=6_2' target='_blank' onclick='poCollect(\"访问升级页面\",{\"触发位置\":\"流程图-添加我的图形\"});buried_point_fn('setTouchPlace',\"我的图形_升级\")'> 升级</a>",
                  'top_error',
                  4000,
                  $('#designer'),
                  true,
            ...
            if (Schema.myShapes.length < 60) {
              ...
              Util.globalTopTip('最多可以添加60个图形', 'top_error', 2000, $('#designer'), true);
            }
          }
          ...
          $('#import_myshape_input')
            ...
            .on('change', function (e) {
              ...
              if (t.indexOf('image') < 0) {
                Util.globalTopTip('此文件不是图片，请重新选择', 'top_error', 2000, $('#myshape_dialog'), true);
                return;
              }
              if (size > 300) {
                Util.globalTopTip('文件大小超出要求，最大300K', 'top_error', 2000, $('#myshape_dialog'), true);
                return;
              }
                ...
                success: function (result) {
                  ...
                  if (result.result == 'type_wrong') {
                    ...
                    if (result.result == 'size_wrong') {
                      Util.globalTopTip('文件大小超出要求，最大2M', 'top_error', 2000, $('#myshape_dialog'), true);
                    } else {
  ...
  hotkey: {
    init: function () {
                  ...
                  if ((j.ctrlKey || j.metaKey) && j.keyCode == 89) {
                    ...
                    if ((j.ctrlKey || j.metaKey) && !j.shiftKey && j.keyCode == 67) {
                      ...
                      if ((j.ctrlKey || j.metaKey) && j.keyCode == 88) {
                        ...
                        if ((j.ctrlKey || j.metaKey) && j.keyCode == 86) {} else {
                                        ...
                                        if (j.keyCode >= 37 && j.keyCode <= 40) {
                                          ...
                                          if (a.length > 0) {
                                            ...
                                            $(document)
                                              ...
                                              .bind('keyup.moveshape', function () {
                                                Model.updateMulti(a);
                                                Designer.events.push('changeLinkers', a);
                                                a = null;
                                            ...
                                            if ((j.ctrlKey || j.metaKey) && j.keyCode == 219) {
                                              ...
                                              if ((j.ctrlKey || j.metaKey) && j.keyCode == 71) {
                                                ...
                                                if ((j.ctrlKey || j.metaKey) && j.keyCode == 76) {
                                                  ...
                                                  if (j.keyCode == 18 && j.ctrlKey) {} else {
                                                    if (j.keyCode == 18) {} else {
                                                      if (j.keyCode == 27) {
                                                        if (!Designer.op.state) {
                                                          ...
                                                          if (
                                                            Designer.op.state == 'creating_free_text' ||
                                                        ...
                                                        if (j.keyCode == 84 && !(j.ctrlKey || j.metaKey)) {
                                                          ...
                                                          if (j.keyCode == 73 && !(j.ctrlKey || j.metaKey)) {
                                                            $('.options_menu').hide();
                                                            $('.color_picker').hide();
                                                            UI.showImageSelect(function (q, e, r) {
                                                              UI.insertImage(q, e, r);
                                                            });
        ...
        .on('paste', function (g) {
          ...
          if (Utils.isChrome() || Utils.isFirefox() || Utils.isSafari()) {
            ...
            if (j) {
              ...
              if (c && c.kind === 'file' && c.type.match(/^image\//i) && h.length != 4) {
                var d = c.getAsFile(),
                  b = new FileReader();
                if (d == null) {
                  return;
                ...
                if (d.size > 2097152) {
                  Util.globalTopTip('图片大小不能超过2M', 'top_error', 3000, $('#designer'), true);
                  return;
                }
                UI.showUploading();
                b.readAsDataURL(d);
                b.onload = function (k) {
                  ...
                  Designer.op.uploadImage(i, function (m) {
                    if (m.result) {
                      ...
                      Util.globalTopTip(
                        "升级到个人版或者团队版后，才可以直接粘贴截图， <a style='color:#fff;letter-spacing:1px;' href='/upgrade?pos=6_2' onclick='buried_point_fn('setTouchPlace',\"粘贴截图_升级\");' target='_blank'>去升级</a>",
                        'top_error',
                        4000,
                        $('#designer'),
                        true,
            ...
            if (j) {
              ...
              if (c && c.kind === 'file' && c.type.match(/^image\//i) && h.length != 4) {
                Util.globalTopTip('该浏览器暂时还不支持粘贴截图', 'top_error', 4000, $('#designer'), true);
              }
            }
            Designer.clipboard.paste();
          }
      ...
      $('#designer_layout')
        ...
        .on('mousewheel DOMMouseScroll', function (d) {
          var c = d.originalEvent.wheelDelta || -d.originalEvent.detail;
          var f = Math.max(-1, Math.min(1, c));
          if (d.ctrlKey || d.metaKey) {
            d.preventDefault();
            var b = {
              x: d.originalEvent.pageX,
              y: d.originalEvent.pageY
            };
            if (f < 0) {
              Designer.zoomOut(b);
            } else {
                                  ...
                                  if (d == 'drawline') {
                                    ...
                                    if (d == 'changelink') {
                                      ...
                                      if (d == 'replace') {
                                        ...
                                        if (d == 'defaultStyle') {
                                          Designer.events.push('setDefaultStyle', c);
                                        } else {
                                          if (d == 'equation') {
                                            var a = this.menuPos.shape;
                                            Designer.op.equation.insertLastEquation(a);
                                          }
                                        }
                                      }
                                    }
  ...
  op: {
    ...
    shapeDraggable: function () {
      ...
      c.bind('mousedown.drag', function (f) {
        ...
        var y = Utils.getSelected();
        var C = Utils.getRelativePos(f.pageX, f.pageY, c);
        var x = true;
        if ((y.length == 1 && y[0].name == 'linker') || (y.length == 1 && y[0].name != 'linker' && y[0].props.angle)) {
          x = false;
        }
        var n = null;
        if (x) {
          n = Utils.toScale(Utils.getShapesBounding(y));
        }
        var v = Utils.getFamilyShapes(y);
        y = y.concat(v);
        var r = Utils.getContainedShapes(y);
        y = y.concat(r);
        var H = Utils.getAttachedShapes(y);
        y = y.concat(H);
        var j = Utils.getCollapsedShapes(y);
        y = y.concat(j);
        var d = [];
        var l = [];
        if (x) {
          ...
          for (var B = Model.orderList.length - 1; B >= 0; B--) {
            var G = Model.orderList[B].id;
            var h = Model.getShapeById(G);
            if (h.attribute && h.attribute.collapseBy) {
              continue;
            }
            if (h.name == 'linker' || d.indexOf(G) >= 0 || h.parent) {
              continue;
            }
            var E = Utils.toScale(Utils.getControlBox([G]));
            E.id = G;
            l.push(E);
          }
        }
        ...
        var p = Model.define.page.height.toScale();
        var m = F[0].props.w;
        var u = F[0].props.h;
        ...
        var w = f.pageX,
          ...
          s = 0,
          q = 0;
        var o = Utils.copyArray(y);
        var z;
        var k = f.metaKey || f.ctrlKey;
        b.bind('mousemove.drag', function (L) {
          UI.hideShapeOptions();
          (s = L.pageX - w), (q = L.pageY - t);
          var K = Utils.getRelativePos(L.pageX, L.pageY, c);
          var N = {
            x: K.x - C.x,
            y: K.y - C.y
          };
          if (k && (Math.abs(N.x) > 1 || Math.abs(N.y) > 1)) {
            if (a == false) {
              Designer.clipboard.copy();
              Designer.clipboard.paste();
              y = Utils.getSelected();
          ...
          if (x) {
            ...
            O.x += N.x;
            O.y += N.y;
            var M = Designer.op.snapLine(O, d, null, null, l);
            N = {
              x: O.x - n.x,
              y: O.y - n.y
            };
            ...
            n.x += N.x;
            n.y += N.y;
            if (F.length == 1 && F[0].groupName == 'boundaryEvent') {
              if (M.attach) {
                F[0].attachTo = M.attach.id;
              } else {
                delete y[0].attachTo;
              }
            }
          ...
          for (var J = 0; J < y.length; J++) {
            ...
            if (I.name == 'linker') {
              if (y.length == 1) {
                z = Designer.op.moveLinkerFocus(I);
              }
              continue;
            } else {
              if ((I.container && A.indexOf(I.container) >= 0) || I.parent) {
                continue;
              }
            }
            if (i) {
          ...
          Designer.op.moveShape(y, N);
          C = K;
          Designer.op.isScroll(L.pageX, L.pageY);
          Designer.events.push('changeLinkers', y);
        });
        $(document)
          .unbind('mouseup.drop')
          .bind('mouseup.drop', function (M) {
            ...
            var I = $(M.target);
            if (
              I.parents('.panel_myshape_container').length > 0 ||
              I.attr('id') == 'add_myshape_icon' ||
              I.parents('.panel_team_container').length > 0
            ) {
            ...
            if (z) {
              Designer.painter.renderLinker(z);
              Model.updateMulti([z]);
            } else {
              Model.updateMulti(y);
            }
            var J = Utils.getControlBox(A);
            Designer.op.changeCanvas(J);
          });
    ...
    shapeResizable: function () {
      $('.shape_controller').bind('mousedown', function (c) {
        ...
        l.bind('mousemove.resize', function (W) {
          ...
          if (C.x != q.x) {
            H = 'X: ' + Math.round(C.x) + '&nbsp;&nbsp;Y: ' + Math.round(C.y) + '<br/>' + H;
          }
            ...
            $(document)
              .bind('mousemove.rotate', function (s) {
                ...
                for (var x = 0; x < i.length; x++) {
                  ...
                  if (z.name != 'linker') {
                    z.props.angle = Math.abs((F + v.props.angle) % (Math.PI * 2));
                    var y = {
                    ...
                    var A = false;
                    if ((Utils.isSelected(z.id) && z.from.id == null) || Utils.isSelected(z.from.id)) {
                      var t = Utils.getRotated(e, v.from, F);
                      z.from.x = t.x;
                      z.from.y = t.y;
                      if (z.from.angle != null) {
                        z.from.angle = Math.abs((v.from.angle + F) % (Math.PI * 2));
                      }
                      A = true;
                    }
                    var u = false;
                    if ((Utils.isSelected(z.id) && z.to.id == null) || Utils.isSelected(z.to.id)) {
    ...
    shapeMultiSelectable: function () {
      ...
      c.off('mousedown.multiselect').on('mousedown.multiselect', function (g) {
        ...
        Designer.op.initScrollPos();
        a.off('mousemove.multiselect').on('mousemove.multiselect', function (j) {
          if (e == null) {
    ...
    editShapeText: function (a, h, v) {
      ...
      if (q.length == 0) {
        q = $("<div contenteditable='true' spellcheck=false id='shape_text_edit'></div>").appendTo('#designer_canvas');
      }
      var p = $('#shape_text_ruler');
      if (p.length == 0) {
        p = $("<div contenteditable='true' spellcheck=false id='shape_text_ruler'></div>").appendTo('#designer_canvas');
      }
      ...
      var t = {
        width: s.w + 'px',
        'z-index': Model.orderList.length + 9999,
      ...
      if (x.text != null) {
        ...
        if (typeof isNewTextV != 'undefined' && !isNewTextV) {
          ...
          o = o
            ...
            .replace(/\"&gt;/gi, '">')
            .replace(/&lt;\/font&gt;/gi, '</font>');
          o = o.replace(/&lt;span/gi, '<span').replace(/&lt;\/span&gt;/gi, '</span>');
      ...
      $('#shape_text_edit')
        ...
        .on('keydown', function (J) {
          ...
          if (J.keyCode == 13 && J.ctrlKey) {
            ...
            if (J.keyCode == 27) {
              ...
              if (J.keyCode == 66 && J.ctrlKey) {
                var C = !b.bold;
                if (a.textBlock.length == 1) {
                  a.fontStyle.bold = C;
                } else {
                  z.fontStyle = $.extend(z.fontStyle, {
                    bold: C
                  });
                }
                b.bold = C;
                Model.update(a);
                var I = C ? 'bold' : 'normal';
                $(this).css('font-weight', I);
                p.css('font-weight', I);
                UI.update();
                J.preventDefault();
              } else {
                if (J.keyCode == 73 && J.ctrlKey) {
                  var C = !b.italic;
                  if (a.textBlock.length == 1) {
                  ...
                  b.italic = C;
                  Model.update(a);
                  var I = C ? 'italic' : 'normal';
                  $(this).css('font-style', I);
                  p.css('font-style', I);
                  UI.update();
                  J.preventDefault();
                } else {
                  if (J.keyCode == 85 && J.ctrlKey) {
                    var C = !b.underline;
                    if (a.textBlock.length == 1) {
                    ...
                    b.underline = C;
                    Model.update(a);
                    var I = C ? 'underline' : 'none';
                    $(this).css('text-decoration', I);
                    p.css('text-decoration', I);
                    J.preventDefault();
                    UI.update();
                    J.preventDefault();
                  } else {
                    if (J.keyCode == 13) {
                      if (Utils.isIE() && !J.shiftKey) {
                        J.preventDefault();
                      }
                    } else {
                      if ((J.keyCode == 67 || J.keyCode == 88) && (J.ctrlKey || J.metaKey)) {
                        var H = window.getSelection().getRangeAt(0),
                          A = H.cloneContents().childNodes;
                        if (A.length > 1 || (A.length == 1 && A[0].nodeType == 1)) {
                          if (J.keyCode == 88) {
                            document.addEventListener('cut', M, true);
                          } else {
                            document.addEventListener('copy', M, true);
                          }
                        }

                        function M(Q) {
                          var O = document.createElement('div');
                          O.innerHTML = '<div class="mind-clipboard"></div>';
                          O.querySelector('.mind-clipboard').appendChild(H.cloneContents());
                          var S = O.innerHTML;
                          var R = O.cloneNode(true);
                          var P = $(R).find('.equation-text');
                          if (P.length > 0) {
                            P.html(P.data('equation'));
                          }
                          Q.clipboardData.setData('Text', $(R).text());
                          Q.clipboardData.setData('text/html', S);
                          if (J.keyCode == 88) {
                            window.getSelection().deleteFromDocument();
                            document.removeEventListener('cut', M, true);
                            if ($(S).find('.equation-text').length > 0) {
                              L.find('.equation-text').each(function (T, U) {
                                $(U).attr('data-index', T);
                              });
                            }
                          } else {
                            document.removeEventListener('copy', M, true);
                          }
                          Q.preventDefault();
                        }
                      } else {
                        if (J.keyCode == 37 || J.keyCode == 39) {
                          if (q.find('.equation-text').length > 0) {
                            ...
                            H.insertNode(i);
                            H.collapse(false);
                            var N = H.startContainer;
                            var D = N.nodeType == 1 ? N : N.parentNode;
                            E(D);
                          }
                          ...
                          function E(Q) {
                            ...
                            for (var O = 0; O < P.length; O++) {
                              if (P[O].nodeType == 3 && P[O].nodeValue == '') {
                                Q.removeChild(P[O]);
                              }
                            }
                            for (var O = 0; O < P.length; O++) {
                              if (P[O].className == 'equation-temp') {
                                if (J.keyCode == 39) {
                                  if (O == P.length - 1) {
                                    F(Q);
                                  } else {
                                    if (P[O + 1].className == 'equation-text') {
                                      Designer.op.equation.equationInit(q, $(P[O + 1]));
                                    } else {
                                      if (P[O + 1].nodeType == 1) {
                                        K(P[O + 1]);
                                      }
                          ...
                          function K(O) {
                            if (O.childNodes.length == 1 && O.childNodes[0].className != 'equation-text') {
                              K(O.childNodes[0]);
                            } else {
                              if (O.nodeType == 1) {
                                if (J.keyCode == 39) {
                          ...
                          if ((J.ctrlKey || J.metaKey) && J.keyCode == 69) {
                            J.preventDefault();
                            var G = $(this);
                            Designer.op.equation.equationInit(G);
                          }
                        }
                      }
                    }
        ...
        .on('paste', function (I) {
          var K = $(this);
          var B = I.clipboardData || I.originalEvent.clipboardData || window.clipboardData,
            D,
            H,
            J;
          if (B) {
            var C = $(B.getData('text/html'));
            var A = $(C[1]).hasClass('mind-clipboard') ? C[1] : $(C[0]).hasClass('mind-clipboard') ? C[0] : C[2];
            D = B.items;
            J = B.types || [];
      ...
      function c() {
        ...
        if ($('#shape_text_edit').length && $('#shape_text_edit').is(':visible')) {
          ...
          a = null;
          Utils.selectShape(Utils.selectIds);
        }
      }
    },
    equation: {
      equationInit: function (k, u) {
        var x;
        $('.pop-editor').hide();
        var t = $('.eq-editor');
        if (t.length == 0) {
        ...
        if (document.selection) {
          ...
          m = document.getSelection().toString().replace(/\n/g, '');
          g = o.htmlText;
        } else {
          var s = window.getSelection();
          o = s.getRangeAt(0);
          m = s.toString().replace(/\n/g, '');
          var l = document.createElement('div');
          l.appendChild(o.cloneContents());
          g = l.innerHTML;
        }
        var b = document.createElement('div');
        b.innerHTML = g;
        var d = $(b).find('.equation-text');
        if (d.length > 0 && !u) {
          u = k.find('.equation-text[data-index=' + d.eq(0).data('index') + ']');
        }
        var y = $('.notranslate');
        if (!u) {
          ...
          o.insertNode(n);
          x = k.clone(true);
          x.find('.equation-temp').remove();
          o.setStart(q, 0);
          o.setEnd(n, 0);
          k.find('.eq-l,.eq-r').remove();
          y.text(m);
          var j = '<span class="equation-text" contenteditable="false"><span></span><span>&#65279;</span></span>';
          var v = $(j)[0];
          v.childNodes[0].appendChild(o.cloneContents());
          var h = $(o.startContainer).parents('.equation-text');
          if (h.length > 0) {
            o.setStartAfter(h.get(0));
          }
          ...
          y.text($(u[0].outerHTML).data('equation'));
        }
        k.find('.equation-text').each(function (z, A) {
          $(A).attr('data-index', z);
        });
        if (!x) {
          x = k.clone(true);
        ...
        u.addClass('cur');
        var w = Utils.getSelected()[0];
        var a = $('#' + w.id);
        var e = parseFloat(a.css('left'));
        var p = parseFloat(a.css('top'));
        t.css({
          left: e + a.outerWidth() / 2 - t.outerWidth() / 2,
          top: p + a.outerHeight()
        }).show();
        $('#designer_canvas').append(
          "<div class='eq-shade' style='position:fixed;top:0;left:0;width:100%;height:100%;z-index:8888'></div>",
        );
        setTimeout(function () {
          Utils.selectText(y.get(0));
        });
        this.equationInitEvent(k, x, u);
      },
      equationInitEvent: function (e, b, f) {
        ...
        h.off()
          ...
          .on('click', '.eg-title span, .eg-title .icons, .eg-con', function (r) {
            var u = [
            ...
            if (o == 'icons') {
              ...
              if (o == 'eg-con') {
                d.html(u[$('.eg-title .on').index()]).trigger('input').focus();
        ...
        d.off()
          ...
          .on('keydown', function (o) {
            ...
            if (o.keyCode == 39 || o.keyCode == 37) {
              ...
              if ((n.anchorOffset == d.text().length && o.keyCode == 39) || (n.anchorOffset == 0 && o.keyCode == 37)) {
                if (c) {
                  setTimeout(function () {
                    m(o);
                    c = true;
    ...
    shapeChangeHeight: function (h) {
      var j = Utils.getShapeFontHeight(h);
      var k = Utils.copy(h.props);
      var b = [h];
      if (h.name == 'interface') {
        var g = j[0] < 30 ? 30 : j[0],
          f = j[1] < 30 ? 30 : j[1];
        h.props.h = g + f;
        h.path[1].actions[0].y = g;
        h.path[1].actions[1].y = g;
        h.textBlock[0].position.h = g;
        h.textBlock[1].position.y = g;
        h.textBlock[1].position.h = 'h-' + g;
        ...
        if (h.name == 'cls') {
          ...
          h.textBlock[0].position.h = g;
          h.textBlock[1].position.y = g;
          h.textBlock[1].position.h = '(h-' + g + ')*' + f + '/' + (f + e);
          h.textBlock[2].position.y = '(h-' + g + ')*' + f + '/' + (f + e) + '+' + g;
          h.textBlock[2].position.h = '(h-' + g + ')*' + e + '/' + (f + e);
          h.path[1].actions[0].y = g;
          h.path[1].actions[1].y = g;
          h.path[1].actions[2].y = '(h-' + g + ')*' + f + '/' + (f + e) + '+' + g;
          h.path[1].actions[3].y = '(h-' + g + ')*' + f + '/' + (f + e) + '+' + g;
          Schema.initShapeFunctions(h);
        }
      }
      var m = Utils.getOutlinkers([h]);
      for (var c = 0; c < m.length; c++) {
        var d = m[c];
        var a = true;
        if (d.from.id == h.id) {
          if (h.props.h != k.h) {
            var l = h.props.y + (h.props.h * (d.from.y - k.y)) / k.h;
            d.from.y = Math.round(l);
            b.push(d);
            a = false;
          }
        }
        if (d.to.id == h.id) {
          if (h.props.h != k.h) {
            var l = h.props.y + (h.props.h * (d.to.y - k.y)) / k.h;
            d.to.y = Math.round(l);
            if (a) {
              b.push(d);
            }
    ...
    shapeLinkable: function (c, a) {
      ...
      function e(k, j) {
        ...
        var h = Model.define;
        if (h.defaultLinkerStyle != null && h.defaultLinkerStyle.linkerStyle != null) {
    ...
    editLinkerText: function (e) {
      ...
      b.unbind()
        .bind('keyup', function () {
          var k = $(this).val();
          var m = k.replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/\n/g, '<br/>');
          h.html(m + '<br/>');
          var i = h.width();
          if (i < 50) {
            i = 50;
      ...
      function c() {
        ...
        if (i.length && i.is(':visible')) {
          ...
          if (Model.intersection[e.id]) {
            Designer.painter.renderLinker(e, null, null, Model.intersection[e.id]);
          } else {
    ...
    canvasScalePoint: function (f, g, l) {
      var c = typeof isView == 'undefined' ? false : isView;
      if (l == 'center') {
        if (c) {
          l = {
            x: $('#viewport').width() / 2 + $('#viewport').offset().left,
            y: $('#viewport').height() / 2 + $('#viewport').offset().top,
          };
        } else {
          if ($('#designer_layout').length) {
            l = {
              x: $('#designer_layout').width() / 2 + $('#designer_layout').offset().left,
              y: $('#designer_layout').height() / 2 + $('#designer_layout').offset().top,
            };
          }
        }
      }
      if (typeof l == 'object') {
        if (c) {
          var j = $('#designer_canvas');
          var k = (l.x - j.offset().left) / f,
            i = (l.y - j.offset().top) / f;
          var b = Math.round(j.position().left - (g - f) * k);
          var h = Math.round(j.position().top - (g - f) * i);
          j.css({
            left: b,
            top: h
          });
          ...
          if ($('#designer_layout').length) {
            var a = $('#designer_layout');
            var k = (l.x - a.offset().left + a.scrollLeft() - Designer.config.pageMargin) / f,
              i = (l.y - a.offset().top + a.scrollTop() - Designer.config.pageMargin) / f;
            var e = Math.round(a.scrollLeft() + (g - f) * k);
            var d = Math.round(a.scrollTop() + (g - f) * i);
            a.scrollLeft(e);
            a.scrollTop(d);
          }
        }
    ...
    moveShape: function (q, h, c) {
      ...
      if (typeof c == 'undefined') {
        ...
        Designer.op.showTip(
          'X: ' + Math.round(k.left.restoreScale()) + '&nbsp;&nbsp;Y: ' + Math.round(k.top.restoreScale()),
        );
      }
    ...
    moveLinkerFocus: function (c) {
      var d = Utils.copy(c);
      var b = Utils.getShapeByPosition(c.to.x.toScale(), c.to.y.toScale(), true);
      var a = Utils.getShapeByPosition(c.from.x.toScale(), c.from.y.toScale(), true);
      Designer.op.hideLinkPoint();
      Utils.hideAnchors();
      ...
      b.bind('mousedown.brokenLinker', function (i) {
        ...
        a.bind('mousemove.brokenLinker', function (k) {
          ...
          var m = 0,
            l = 0;
          if (Math.abs(f.x - e.x) < 1) {
            var o = Designer.op.brokenLinkerChangableDisabled(d, c, f.x + p.x, null);
            m = o - f.x;
            f.x = o;
            e.x = o;
          } else {
            var n = Designer.op.brokenLinkerChangableDisabled(d, c, null, f.y + p.y);
            l = n - f.y;
            f.y = n;
            e.y = n;
          }
    ...
    resetLinkerText: function (d) {
      ...
      if (b != null && b.x != null) {
        ...
        a.css({
          left: b.x.toScale() - g.left - a.width() / 2,
          top: e
        });
      }
    },
    ...
    setDefaultStyle: function () {
      ...
      if (c.length == 1) {
        var a = c[0];
        if (a.name == 'standardImage' || a.name == 'note' || a.name == 'uiNote') {
          return 'error';
        }
        if (a.category != null && (a.category.indexOf('ios_') >= 0 || a.category.indexOf('andriod_') >= 0)) {
          return 'error';
        }
        ...
        if (
          a.category != null &&
          (a.category.indexOf('aws_') >= 0 || a.category.indexOf('azure') >= 0 || a.category.indexOf('aws2019') >= 0)
        ) {
          return 'error';
        }
        if (a.category != null && (a.category.indexOf('ali_') >= 0 || a.category.indexOf('cisco_') >= 0)) {
          return 'error';
        }
        var d = {},
          b = 'shape';
        if (a.name == 'linker') {
          b = 'linker';
          d = {
            linkerStyle: Utils.copy(a.lineStyle)
          };
        } else {
          d = {
    ...
    snapLine: function (u, v, F, j, e) {
        ...
        for (var y = Model.orderList.length - 1; y >= 0; y--) {
          ...
          if (c.name != 'linker' && c.id != C.id) {
            ...
            if (
              m.attach == null &&
              w.angle == 0 &&
              (c.groupName == 'task' || c.groupName == 'callActivity' || c.groupName == 'subProcess')
            ) {
              var A = {
                x: w.x - f,
                y: w.y - f,
      ...
      if (m.attach == null) {
        ...
        for (var y = e.length - 1; y >= 0; y--) {
          ...
          if (!Utils.rectInRect(G, u) && !Utils.rectInRect(u, G) && !Utils.rectCross(G, u)) {
          ...
          if (m.container == null) {
            ...
            if (c.attribute && c.attribute.container) {
              if (Utils.rectInRect(u, Utils.toScale(c.props))) {
                m.container = c;
              }
            }
          }
    ...
    drawSnpLine: function (b, g, a, i) {
      ...
      if (Math.abs(e.x - h.x) < 2) {
        ...
        var d = $("<div class='designer_op_snapline_v " + i + "'>" + f + '</div>').appendTo(c);
        d.css({
        ...
        if (Math.abs(e.y - h.y) < 2) {
          if (i == 'line') {
            f = "<span class='text'>" + Math.round(h.x.restoreScale() - e.x.restoreScale()) + 'px</span>';
          } else {
            f =
          ...
          var d = $("<div class='designer_op_snapline_h " + i + "'>" + f + '</div>').appendTo(c);
          d.css({
    ...
    linkDashboard: function (h) {
      ...
      if ($('#panel_' + c).length != 0) {
        ...
        if (j.length == 0) {
          j = $(
            "<div id='shape_dashboard_" + c + "' class='shape_dashboard menu' style='display:none;'></div>",
          ).appendTo('#designer_canvas');
          ...
          function g(p, s) {
            ...
            var o = $(r).appendTo(j);
            if (s) {
              o.append("<div class='group_icon link_shape_icon' group='" + s + "'></div>");
            }
            var q = o.children()[0];
    ...
    groupDashboard: function (k, d, j, c) {
      ...
      if (h.length == 0) {
        h = $("<div id='shape_group_dashboard_" + k + "' class='group_dashboard menu'></div>").appendTo(
          '#designer_canvas',
        );
    ...
    changeShapeProps: function (g, h, e) {
      function b(j) {
        ...
        if (typeof h.w != 'undefined' || typeof h.h != 'undefined' || typeof h.angle != 'undefined') {
    ...
    changeCanvas: function (e) {
      if (Model.define.page.showPrintLine) {
        var f = Utils.getPrintPageSize();
        var d = Model.define.page.width,
          i = Model.define.page.height;
        if (f.width != d || f.height != i) {
          Designer.setPageStyle({
            width: f.width,
            height: f.height
          });
      ...
      if (Model.define.page.orientation == 'portrait') {
        var d = Model.define.page.width;
        var i = Model.define.page.height;
        var a = e.x;
        var g = e.y;
        var b = e.w;
        var c = e.h;
        if (a + b > d) {
          Designer.setPageStyle({
            width: Math.floor(a + b) + 30
          });
        }
        if (g + c > i) {
          Designer.setPageStyle({
            height: Math.floor(g + c) + 30
          });
        }
      }
    },
      ...
      if (localStorage.clipboard) {
        ...
        if (CLB.clientId != c) {
          ...
          if (typeof o == 'undefined') {
            ...
            var u = $('#designer_layout').scrollLeft();
            var l = u - Designer.config.pageMargin + $('#designer_layout').width().restoreScale() / 2;
            o = l;
            m = k;
          }
        }
      ...
      if (this.elements.length > 0) {
        ...
        for (var z = 0; z < this.elements.length; z++) {
          ...
          if (a.name != 'linker') {
            var D;
            var a = this.elements[z];
            a.props.zindex = Model.maxZIndex + (z + 1);
            var r = this.presetedIds[a.id];
            if (this.plus || typeof o != 'undefined') {
              a.props.x += B;
              a.props.y += A;
            }
            D = Utils.copy(a);
            for (var w = 0; w < D.dataAttributes.length; w++) {
              var t = D.dataAttributes[w];
              t.id = Utils.newId();
            }
            D.id = r;
            if (D.children) {
              for (var q = 0; q < D.children.length; q++) {
                var v = D.children[q];
                D.children[q] = this.presetedIds[v];
              }
            }
            if (D.parent) {
              D.parent = this.presetedIds[D.parent];
            }
            h.push(D);
            d.push(r);
            if (a.group) {
        ...
        for (var z = 0; z < this.elements.length; z++) {
          ...
          if (a.name == 'linker') {
            ...
            if (this.plus || typeof o != 'undefined') {
              ...
              a.to.x += B;
              a.to.y += A;
              for (var C = 0; C < a.points.length; C++) {
                var s = a.points[C];
                s.x += B;
                s.y += A;
              }
            ...
            if (a.from.id != null) {
              D.from.id = this.presetedIds[a.from.id];
            }
            if (a.to.id != null) {
              D.to.id = this.presetedIds[a.to.id];
            }
            D.id = r;
            h.push(D);
            d.push(r);
        ...
        this.plus = true;
        Util.shapesCount();
        var n = Utils.getControlBox(d);
        Designer.op.changeCanvas(n);
      }
    },
    duplicate: function () {
      this.copy();
  ...
  painter: {
    ...
    renderPath: function (n, l, p, b, f) {
      ...
      for (var g = 0; g < p.length; g++) {
                ...
                if (c.row == 0) {
                  if (a.color == '255,255,255') {
                    m = $.extend({}, {
                      type: 'solid',
                      color: '240, 240, 240'
                    }, l.theme.row[0].fillStyle, c.fillStyle);
                  } else {
                ...
                if (l.theme.name == 'striping') {
                  if (l.theme.row) {
                    if (a.color == '255,255,255') {
                      ...
                      m = $.extend({}, a, l.theme.row[c.row % 2].fillStyle, c.fillStyle);
                    }
                  }
                  if (l.theme.column) {
                    if (a.color == '255,255,255') {
                      ...
                      m = $.extend({}, a, l.theme.column[c.column % 2].fillStyle, c.fillStyle);
                    }
                  }
                }
              }
        ...
        var d = false;
        if (h.lineStyle != 'solid' && typeof n.lineDashOffset == 'undefined' && m.type != 'none') {
          d = true;
        }
        if (d) {
          n.save();
          ...
          if (h.lineStyle == 'dashed') {
            ...
            if (h.lineStyle == 'dot') {
              ...
              if (h.lineStyle == 'dashdot') {
                this.setLineDash(n, [h.lineWidth * 5, h.lineWidth * 2, h.lineWidth, h.lineWidth * 2]);
              } else {
                delete n.webkitLineDash;
              }
            }
    ...
    drawPanelItemByShape: function (canvas, myshape, config) {
      ...
      for (var i = 0; i < myshape.elements.length; i++) {
        ...
        if (shape.name == 'linker') {
          ...
          ctx.lineJoin = 'round';
          if (isPanel) {
            shape.lineStyle.lineWidth = shape.lineStyle.lineWidth > 0.5 ? shape.lineStyle.lineWidth : 0.5;
          } else {
          ...
          ctx.translate(-(props.w / 2), -(props.h / 2));
          if (isPanel) {
            shape.lineStyle.lineWidth = shape.lineStyle.lineWidth > 1 ? shape.lineStyle.lineWidth : 1;
          } else {
          ...
          if (shape.name.indexOf('Lane') > -1) {
            ...
            shape.getPath = eval('(function(){return ' + shape.getPath + ' })()');
          }
      ...
      if (r.lineStyle == 'dashed') {
        ...
        if (r.lineStyle == 'dot') {
          ...
          if (r.lineStyle == 'dashdot') {
            this.setLineDash(m, [r.lineWidth * 5, r.lineWidth * 2, r.lineWidth, r.lineWidth * 2]);
          }
        }
      }
    ...
    renderShape: function (b) {
      ...
      if (s.length == 0) {
        ...
        s = $("<div id='" + b.id + "' class='shape_box'><canvas class='shape_canvas'></canvas></div>").appendTo(m);
      }
      if (typeof isView != 'undefined' && b.link) {
        ...
        var o = $("<a title='" + b.link + "' target='_blank' href='" + q + "' class='shape_link'></a>");
        o.appendTo(s);
      }
      if (b.attribute && b.attribute.collapseBy) {
        s.hide();
    ...
    fillShape: function (f, i, h, e) {
      ...
      if (h.type == 'solid') {
        ...
        if (h.type == 'gradient') {
          ...
          if (h.type == 'image') {
            var a;
            if (h.fileId && h.fileId.indexOf('qiniu/') >= 0) {
              a = h.fileId.replace(/^qiniu/, 'http://7xpicf.com1.z0.glb.clouddn.com');
            } else {
              if (h.fileId && h.fileId.indexOf('http') == 0) {
                a = h.fileId;
                ...
                if (h.fileId && h.fileId.indexOf('/images/') >= 0) {
                  a = h.fileId;
                  if (localRuntime) {
      ...
      for (var o = 0; o < b.length; o++) {
        ...
        if (typeof isNewTextV != 'undefined' && !isNewTextV) {
          ...
          j = j
            ...
            .replace(/\"&gt;/gi, '">')
            .replace(/&lt;\/font&gt;/gi, '</font>');
          j = j.replace(/&lt;span/gi, '<span').replace(/&lt;\/span&gt;/gi, '</span>');
    ...
    renderLinker: function (D, d, U, r, J) {
      ...
      if (L.length == 0) {
        ...
        L = $(
          "<div id='" + D.id + "' class='shape_box linker_box'><canvas class='shape_canvas'></canvas></div>",
        ).appendTo(e);
      }
      ...
      if (O.lineStyle == 'dashed') {
        ...
        if (O.lineStyle == 'dot') {
          ...
          if (O.lineStyle == 'dashdot') {
            this.setLineDash(u, [O.lineWidth * 5, O.lineWidth * 2, O.lineWidth, O.lineWidth * 2]);
          }
        }
      }
    ...
    renderLinkerText: function (i, m, d) {
      ...
      b.show();
      var l = i.text.replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/\n/g, '<br/>');
      b.html(l + '<br/>');
      var n = null;
      if (i.textPos != null) {
        if (m || d) {
      ...
      if (n != null && n.x != null) {
        ...
        b.css({
          left: n.x.toScale() - e.left - b.width() / 2,
          top: j
        });
      }
    },
...
window.Utils = {
  ...
  newId: function () {
    return Utils.randomString(10) + Utils.randomNumber(6);
  },
  ...
  randomString: function (a, d) {
    d = d || 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
    var b = '';
    for (var c = 0; c < a; c++) {
      ...
      if (s.name == 'linker') {
        if (s.textPos != null) {
          if (s.textPos.pos == 'top') {
            S.y = Math.min(S.y, s.textPos.y - aa.children('.linker_text').outerHeight());
          } else {
        ...
        if (k.isPointInPath(V, C)) {
          ...
          if (!s.attribute || typeof s.attribute.linkable == 'undefined' || s.attribute.linkable) {
            var r = Utils.getPointAngle(R, O.restoreScale(), N.restoreScale(), P);
            if (r != null) {
              var t = null;
              var b = {
                angle: r
      ...
      if (l.name == 'linker') {
        if (this.isLocked(l.id)) {
          ...
          if (Model.intersection[l.id]) {
            Designer.painter.renderLinker(l, null, null, Model.intersection[l.id]);
          } else {
    ...
    for (var b = 0; b < c.length; b++) {
      ...
      if (a.name == 'linker') {
        if (Model.intersection[a.id]) {
          Designer.painter.renderLinker(a, null, null, Model.intersection[a.id]);
        } else {
  ...
  showAnchors: function (i) {
    //window.postMessage({name:"toshowItemTask",item:JSON.stringify(i)});
  ...
  getLinkerPoints: function (w, e) {
    var D = [];
    if (w.linkerType == 'broken') {
      var F = Math.PI;
      var B = w.from;
      var d = w.to;
      var p = Math.abs(d.x - B.x);
      var G = Math.abs(d.y - B.y);
      var u = 30;
      e = e ? 20 : 0;
      var r = B.id != null && Model.getShapeById(B.id);
      var h = d.id != null && Model.getShapeById(d.id);
      if (r && h) {
        var c = this.getAngleDir(B.angle);
        ...
        if (c == 1 && b == 1) {
          ...
          var j = Model.getShapeById(i.id).props;
          var A = Model.getShapeById(k.id).props;
          if (k.x >= j.x - u && k.x <= j.x + j.w + u) {
            var q;
            if (k.x < j.x + j.w / 2) {
              q = j.x - u;
            } else {
        ...
        if (r || h) {
          ...
          var f = Model.getShapeById(i.id).props;
          if (E >= F / 4 && E < (F / 4) * 3) {
            if (k.y < i.y - e) {
              if (p >= G) {
                D.push({
      ...
      if (w.linkerType == 'curve') {
        var B = w.from;
        var d = w.to;
        var g = this.measureDistance(B, d);
        var m = g * 0.4;

        function v(H, I) {
          if (H.id != null) {
            return {
              x: H.x - m * Math.cos(H.angle),
              y: H.y - m * Math.sin(H.angle)
            };
          } else {
            var J = Math.abs(H.y - I.y);
            var y = Math.abs(H.x - I.x);
            var K = Math.atan(J / y);
            var x = {};
            if (H.x <= I.x) {
              x.x = H.x + m * Math.cos(K);
            } else {
              x.x = H.x - m * Math.cos(K);
            }
            if (H.y <= I.y) {
              x.y = H.y + m * Math.sin(K);
            } else {
              x.y = H.y - m * Math.sin(K);
            }
            return x;
  ...
  getShapeFontStyle: function (c, b) {
    ...
    if (b == false || !Model.define.theme || !Model.define.theme.shape) {
      ...
      return $.extend({}, Schema.shapeDefaults.fontStyle, Model.define.theme.shape.fontStyle, c);
    }
  },
  getShapeFontHeight: function (b) {
    var c = b.getTextBlock();
    var a = Utils.getShapeFontStyle(b.fontStyle);
    var e = $('#shape_text_ruler');
    if (e.length == 0) {
      e = $("<div contenteditable='true' spellcheck=false id='shape_text_ruler'></div>").appendTo('#designer_canvas');
    }
    ...
    function d(g) {
      ...
      var h = {
        width: f.w + 'px',
        'z-index': Model.orderList.length + 9999,
        'line-height': 100 * k.lineHeight + '%',
        'font-size': k.size + 'px',
        'font-family': k.fontFamily,
        'font-weight': k.bold ? 'bold' : 'normal',
        'font-style': k.italic ? 'italic' : 'normal',
        'text-align': k.textAlign,
        color: 'rgb(' + k.color + ')',
        'text-decoration': k.underline ? 'underline' : 'none',
        'word-wrap': 'break-word',
      };
      e.css(h);
      var j = g.text.replace(/\n/g, '<br>').replace(/\t/g, '&nbsp;').replace(/\  /g, '&nbsp;&nbsp;');
      return e.html(j).height();
    }
  },
  ...
  getSeparatorsByPool: function (e) {
    ...
    for (var c = 0; c < a; c++) {
      ...
      if (b.name.indexOf('Separator') > -1 && b.name.indexOf('SeparatorBar') < 0) {
        d.push(b);
      }
    }
    return d;
  ...
  filterXss: function (a) {
    ...
    a = a.replace(/&lt;font&gt;/g, '<font').replace(/&lt;\/font&gt;/g, '</font>');
    return a;
  },
  showError: function () {
    var a = $('#canvas-error');
  ...
  popEditor: {
    ...
    renderMenuPos: function (e) {
      ...
      a.css({
        ...
        top: c + b.outerHeight() + (b.outerHeight().toScale() - b.outerHeight()) / 2,
      }).show();
    },
  ...
  toast: function (c, d, b) {
    var a = $('.toast-con');
    if (a.length == 0) {
      a = $('<div class="toast-con moving"><span>' + c + '</span></div>').appendTo('#ui_container');
    }
    d = $(d);
    var e = d.offset();
    a.css({
  ...
  getPrintPageSize: function () {
    var a = flow2svg.calcCanvasSize();
    var b = (a.minX + a.w).restoreScale() - 10,
      c = (a.minY + a.h).restoreScale() - 10;
    if (Model.define.page.orientation == 'landscape') {
      b = [c, (c = b)][0];
    }
    b = Math.max(1050, Math.ceil(b / 1050) * 1050);
    c = Math.max(1485, Math.ceil(c / 1485) * 1485);
    return {
      width: b,
      height: c
    };
...
window.MessageSource = {
  ...
  undoStack: {
    ...
    push: function (b, a) {
      ...
      const oldData = Utils.copy(Model.define);
      window.parent.postMessage({
        iframeMessage: JSON.stringify(oldData),
        type: 'stepOldData'
      }, '*');
    },
    pop: function () {
      console.log("fhsdjfghdjfgh")
      // ======
      console.log(this.stack, '撤回列表')
  ...
  undo: function (stack) {
    var a = stack ? stack : this.undoStack.pop();
    if (a == null) {
      return;
    }
    this.doWithoutUndo(function () {
      ...
      for (var g = 0; g < a.length; g++) {
        ...
        if (e.action == 'create') {
          Utils.unselect();
          Model.remove(e.content, false);
          $('.shape_dashboard.menu').hide();
          Designer.events.push('changeLinkers');
        } else {
          if (e.action == 'update') {
            var c = e.content.shapes;
            var f = [];
            for (var j = 0; j < c.length; j++) {
              var h = c[j];
              if (Model.define.elements[h.id]) {
                if (h.name == 'linker') {
                  if (h.from.id && Model.define.elements[h.from.id] == null) {
                    h.from.id = null;
                  }
                  if (h.to.id && Model.define.elements[h.to.id] == null) {
                    h.to.id = null;
                  }
                  f.push(h);
                  Designer.painter.renderShape(h);
            ...
            if (e.action == 'remove') {
              ...
              for (var j = 0; j < c.length; j++) {
                ...
                if (!Model.define.elements[h.id]) {
                  if (h.name == 'linker') {
                    if (h.from.id && Model.define.elements[h.from.id] == null && k[h.to.id] == null) {
                      h.from.id = null;
                    }
                    if (h.to.id && Model.define.elements[h.to.id] == null && k[h.to.id] == null) {
                      h.to.id = null;
                    }
                    f.push(h);
                    Designer.painter.renderShape(h);
              ...
              if (e.action == 'updatePage') {
                var d = Model.define.page.lineJumps;
                Model.updatePage(e.content.page);
                Designer.events.push('resetBrokenLinker', d);
              } else {
                if (e.action == 'setTheme') {
                  Model.setTheme(e.content.theme);
                  Designer.events.push('changeLinkers');
                } else {
                  if (e.action == 'setDefaultStyle') {
                    Model.setDefaultStyle(e.content.type, e.content.style);
                  } else {
                    if (e.action == 'update_with_beautify') {
                      Model.updateWithBeautify(e.content.content, e.content.updates, e.content.oldTheme);
                    }
                  }
                }
              }
  ...
  executeMessages: function (g, m) {
    ...
    for (var h = 0; h < g.length; h++) {
      ...
      if (d.action == 'create') {
        ...
        for (var l = 0; l < b.length; l++) {
          var k = b[l];
          if (!Model.define.elements[k.id]) {
            if (k.name == 'linker') {
              if (k.from.id && Model.define.elements[k.from.id] == null && o[k.from.id] == null) {
                k.from.id = null;
              }
              if (k.to.id && Model.define.elements[k.to.id] == null && o[k.to.id] == null) {
                k.to.id = null;
              }
              e.push(k);
              Designer.painter.renderShape(k);
        ...
        if (d.action == 'update') {
          var n = d.content.updates;
          var j = [];
          for (var l = 0; l < n.length; l++) {
            var f = n[l];
            if (Model.define.elements[f.id]) {
              if (f.name == 'linker') {
                if (f.from.id && Model.define.elements[f.from.id] == null) {
                  f.from.id = null;
                }
                if (f.to.id && Model.define.elements[f.to.id] == null) {
                  f.to.id = null;
                }
                j.push(f);
                Designer.painter.renderShape(f);
                ...
                if (d.action == 'setDefaultStyle') {
                  ...
                  if (d.action == 'clearDefaultStyle') {
                    ...
                    if (d.action == 'update_with_beautify') {
                      Model.updateWithBeautify(d.content.updates, d.content.content, d.content.theme);
                    }
                  }
                }
              }
