window.UI = {
  ...
  init: function () {
    ...
    function b() {
      var f = $('.toolbar_con').width(),
        g = $('.toolbar_con_box').width();
      if (f > g) {
        $('.toolbar_con_box').css({ left: 0, right: 'initial' });
        $('.toolbar_con_left').hide();
        $('.toolbar_con_right').hide();
      } else {
        if ($('.toolbar_con_box').position().left !== 0) {
          $('.toolbar_con_left').show();
          $('.toolbar_con_right').hide();
        } else {
          $('.toolbar_con_right').show();
          $('.toolbar_con_left').hide();
        }
      }
    }
    ...
    $('.toolbar_con_right')
      .off()
      .on('click', function (f) {
        $('.toolbar_con_box').css({ left: 'initial', right: '0' });
        b();
      });
    Designer.events.push('isMember', function (f) {
      flowIsMember = f;
    });
    ...
    $('.diagram_title').bind('click', function () {
      ...
      $('#title_container').children('input').val(f).select();
      $('#title_container')
        .children('input')
        .bind('blur', function () {
          var g = $.trim($(this).val());
    ...
    $('#bar_font_family').button({
      onMousedown: function () {
        $('#font_list').dropdown({
          ...
          onSelect: function (h) {
            var g = h.text();
            Designer.setFontStyle({ fontFamily: g });
    ...
    $('#bar_font_italic').button({
      onClick: function () {
        ...
        Designer.setFontStyle({ italic: f });
    ...
    $('#bar_font_underline').button({
      onClick: function () {
        ...
        Designer.setFontStyle({ underline: f });
    ...
    $('#bar_font_height').button({
      onMousedown: function () {
        $('#font_height_list').dropdown({
          ...
          onSelect: function (j) {
            var i = parseFloat(j.text());
            Designer.setFontStyle({ lineHeight: i });
            Utils.selectShape(Utils.selectIds);
          },
        });
        var g = Utils.getSelected()[0];
        var h = Utils.getShapeFontStyle(g.fontStyle);
        var f = h.lineHeight;
        $('#font_height_list')
          .children()
          .each(function () {
            if (parseFloat($(this).text()) == f) {
    ...
    $('#bar_fill').button({
      onMousedown: function () {
        ...
        $.colorpicker({
          target: $('#bar_fill'),
          onSelect: function (g) {
            Designer.setFillStyle({ type: 'solid', color: g });
            $('#bar_fill').button('setColor', g);
          },
          color: f,
          extend:
            "<div id='bar_fill_gradient' title='渐变' class='extend_item'><img src='./assets/images/color_picker/bg_gradient.svg'></div>",
        });
        $('#bar_fill_gradient')
          .unbind()
          .bind('click', function () {
        ...
        $('#bar_fill_img')
          ...
          .bind('click', function () {
            UI.showImageSelect(function (i, g, j) {
              Designer.setFillStyle({ type: 'image', fileId: i, imageW: g, imageH: j });
            });
            $('#color_picker').dropdown('close');
          });
      },
    ...
    $('#bar_line_color').button({
      onMousedown: function () {
        ...
        $.colorpicker({
          ...
          onSelect: function (g) {
            Designer.setLineStyle({ lineColor: g });
    ...
    $('#bar_line_width').button({
      onMousedown: function () {
        $('#line_width_list').dropdown({
          ...
          onSelect: function (j) {
            var i = parseInt(j.text());
            Designer.setLineStyle({ lineWidth: i });
          },
        });
        var g = Utils.getSelected()[0];
        var f;
    ...
    $('#bar_beginarrow').button({
      onMousedown: function () {
        ...
        var h = f.beginArrowStyle;
        var i = $('#beginarrow_list').children('li[arrow=' + h + ']');
        $('#beginarrow_list').dropdown('select', i);
      },
    });
    $('#bar_endarrow').button({
      onMousedown: function () {
        ...
        var h = f.endArrowStyle;
        var i = $('#endarrow_list').children('li[arrow=' + h + ']');
        $('#endarrow_list').dropdown('select', i);
      },
    });
    $('#bar_front').button({
      onClick: function () {
    ...
    $('#bar_beautify').button({
      onClick: function () {
        ...
        var f = Beautify.getBeautifyDefinition();
        if (f.updates.length > 0) {
          Model.updateWithBeautify(f.updates, f.content, f.theme);
          Utils.unselect();
          UI.showTip('美化完成');
          setTimeout(function () {
            UI.hideTip();
    ...
    $('#bar_collapse').button({
      onClick: function () {
        var f = UI.toogleTitleBar();
        CLB.setConfig('showToolbar', f);
      },
    });
    $('#bar_lane_num').spinner({
      ...
      onChange: function (j) {
        var f = false;
        for (var g = 0; g < Schema.categories.length; g++) {
          var h = Schema.categories[g];
          if (h.name == 'lane') {
            f = true;
          }
        }
        console.log(f,j,"aghagah");
        if (f) {
          Designer.setLaneNum(j);
        } else {
    ...
    $('#bar_grid_row').spinner({
      ...
      step: 1,
      onChange: function (h) {
        var f = Utils.getSelected()[0].path.length;
        var g = { type: 'row', val: h - f };
    ...
    $('#bar_grid_column').spinner({
      ...
      step: 1,
      onChange: function (h) {
        var f = Utils.getSelected()[0].path[0].length;
        var g = { type: 'column', val: h - f };
        Designer.setGridNum(g);
      },
    });
    $('#bar_column_before').button({
      onClick: function () {
        var f = Utils.getSelected()[0].path[0].length;
        var h = Utils.gridSelectObj;
        var g = h.columns[h.columns.length - 1] + 1;
        var j = h.rows[h.rows.length - 1] + 1;
        var i = { type: 'column', val: 1, columnIndex: g, rowIndex: j, direction: 'before' };
        Designer.setGridNum(i);
      },
    });
    $('#bar_column_after').button({
      onClick: function () {
        ...
        var h = { type: 'column', val: 1, columnIndex: f, rowIndex: i, direction: 'after' };
    ...
    $('#bar_row_before').button({
      onClick: function () {
        ...
        var f = g.columns[g.columns.length - 1] + 1;
        var i = g.rows[g.rows.length - 1] + 1;
        var h = { type: 'row', val: 1, columnIndex: f, rowIndex: i, direction: 'before' };
        Designer.setGridNum(h);
      },
    });
    $('#bar_row_after').button({
      onClick: function () {
        ...
        var f = g.columns[g.columns.length - 1] + 1;
        var i = g.rows[g.rows.length - 1] + 1;
        var h = { type: 'row', val: 1, columnIndex: f, rowIndex: i, direction: 'after' };
        Designer.setGridNum(h);
      },
    });
    $('#bar_del_column').button({
      onClick: function () {
        ...
        var f = g.columns[g.columns.length - 1] + 1;
        var i = g.rows[g.rows.length - 1] + 1;
        var h = { type: 'column', val: -1, columnIndex: f, rowIndex: i };
        Designer.setGridNum(h);
      },
    });
    $('#bar_del_row').button({
      onClick: function () {
        ...
        var f = g.columns[g.columns.length - 1] + 1;
        var i = g.rows[g.rows.length - 1] + 1;
        var h = { type: 'row', val: -1, columnIndex: f, rowIndex: i };
        Designer.setGridNum(h);
      },
    });
    ...
    $('#export_btn').bind('click', function (g) {
      ...
      c(f);
      g.stopPropagation();
      buried_point_fn('click', 'export_click', '流程图编辑器', {
        按钮名称: '下载',
        文件类型: buried_point.visit.option['文件类型'],
      });
    });
    if (navigator.userAgent.indexOf('app/tencent_wemeet') > -1) {
      $('#wemeet_share_btn').css('display', 'flex');
      $('#wemeet_share_btn').show();
    } else {
      $('#share_btn').show();
    }
    function c(g) {
      var j = g.attr('menu');
      if (g.hasClass('readonly')) {
        return;
      }
      ...
      if (j == 'bar_list_page') {
        $('#bar_page_color')
          ...
          .on('mouseenter', function () {
            $.colorpicker({
              ...
              trans: true,
              onSelect: function (k) {
                Designer.setPageStyle({ backgroundColor: k });
              },
            });
          });
        $('#bar_page_color')
        ...
        var h = $('#bar_list_pagesize').children(
          'li[w=' + Model.define.page.width + '][h=' + Model.define.page.height + ']',
        );
        if (h.length > 0) {
          $('#bar_list_pagesize').dropdown('select', h);
        } else {
        ...
        h = $('#bar_list_padding').children('li[p=' + Model.define.page.padding + ']');
        $('#bar_list_padding').dropdown('select', h);
        h = $('#bar_list_gridsize').children('li[s=' + Model.define.page.gridSize + ']');
        $('#bar_list_gridsize').dropdown('select', h);
        if (Model.define.page.showGrid) {
          $('#bar_list_page')
            .children('li[ac=set_page_showgrid]')
            .prepend("<div class='ico ico_selected diagraming-icons'>&#xe75c;</div>");
        } else {
          $('#bar_list_page').children('li[ac=set_page_showgrid] > .ico_selected').remove();
        }
        if (Model.define.page.showPrintLine) {
          $('#bar_list_page')
            .children('li[ac=set_page_showPrintLine]')
            .prepend("<div class='ico ico_selected diagraming-icons'>&#xe75c;</div>");
        } else {
          $('#bar_list_page').children('li[ac=set_page_showPrintLine] > .ico_selected').remove();
        }
        if (Model.define.page.lineJumps) {
          ...
          $('#bar_list_page').children('li[ac=set_page_linejumps] > .ico_selected').remove();
        }
        var f = 'portrait';
        if (Model.define.page.orientation) {
          f = Model.define.page.orientation;
        }
        var i = $('#bar_list_orientation').children('li[ori=' + f + ']');
        $('#bar_list_orientation').children().menuitem('unselect');
        i.menuitem('select');
      } else {
        if (j == 'bar_list_view') {
          var h = $('#bar_list_view').children(".static[zoom='" + Designer.config.scale + "']");
          if (h.length) {
            $('#bar_list_page').dropdown('select', h);
          }
        }
      }
    }
    function e(u) {
      ...
      if (k == 'rename') {
        ...
        if (k == 'close') {
          ...
          if (k == 'saveAs') {
            UI.showSaveAs();
            buried_point_fn('setTouchPlace', '克隆此文件');
            buried_point_fn('click', 'new_function', '流程图编辑器', {
              操作: '新建文件',
              新建类型: '复制克隆文件',
              文件类型: '流程图',
            });
                                  ...
                                  if (k == 'zoom') {
                                    ...
                                    if (v == 'in') {
                                      ...
                                      if (v == 'out') {
                                        ...
                                        var m = parseFloat(v);
                                        Designer.setZoomScale(m, 'center');
                                      }
                                    }
                                  } else {
                                    if (k == 'insert') {
                                      ...
                                      if (r == 'text') {
                                        ...
                                        if (r == 'image') {
                                          UI.showImageSelect(function (s, p, y) {
                                            UI.insertImage(s, p, y);
                                          });
                                        } else {
                                          if (r == 'line') {
                                            ...
                                            if (r == 'equation') {
                                              var o = Utils.getSelected()[0];
                                              Designer.op.equation.insertLastEquation(o);
                                            }
                                          }
                                        }
                                      }
                                      ...
                                      if (k == 'set_page_size') {
                                        var t = parseInt(u.attr('w'));
                                        var l = parseInt(u.attr('h'));
                                        Designer.setPageStyle({ width: t, height: l });
                                      } else {
                                        if (k == 'set_page_padding') {
                                          var i = parseInt(u.attr('p'));
                                          Designer.setPageStyle({ padding: i });
                                        } else {
                                          if (k == 'set_page_orientation') {
                                            var g = u.attr('ori');
                                            if (Model.define.page.showPrintLine) {
                                              Model.define.page.orientation = g;
                                              ...
                                              Designer.setPageStyle({ orientation: g });
                                            }
                                          } else {
                                            if (k == 'set_page_showgrid') {
                                              ...
                                              if (k == 'set_page_gridsize') {
                                                var x = parseInt(u.attr('s'));
                                                Designer.setPageStyle({ gridSize: x });
                                              } else {
                                                if (k == 'set_page_showPrintLine') {
                                                  if (Model.define.page.showPrintLine) {
                                                    u.menuitem('unselect');
                                                    Designer.setPageStyle({ showPrintLine: false });
                                                  } else {
                                                    u.menuitem('select');
                                                    var f = Utils.getPrintPageSize();
                                                    Designer.setPageStyle({
                                                  ...
                                                  if (k == 'set_page_linejumps') {
                                                    var j = false;
                                                    if (Model.define.page.lineJumps) {
                                                      j = true;
                                                      u.menuitem('unselect');
                                                      Designer.setPageStyle({ lineJumps: false });
                                                    } else {
                                                      u.menuitem('select');
                                                      j = false;
                                                      Designer.setPageStyle({ lineJumps: true });
                                                    }
                                                    Designer.events.push('resetBrokenLinker', j);
                                                  } else {
                                                    if (k == 'front') {
                                                      Designer.layerShapes('front');
                                                    } else {
    ...
    $('#brain-power')
      ...
      .on('click', function () {
        ...
        $('#designer')
          ...
          .on('mousedown.clsoeAi', function (j) {
            smartAiHelpCon.close();
          });
      });
  },
  ...
  update: function () {
    ...
    if (k == 0) {
      ...
      if (a.name.indexOf('Pool') > -1) {
        ...
        if(a.name.includes('verticalPool')){
          $('#showLang').css('display', 'block');
          if(a.property === 'double' || a.name.indexOf('verticalPool'))
            window.postMessage({type:'findsxyd'}, '*');
          else
            window.postMessage({type:'notsxyd'}, '*');
        }
        var r = Utils.getLanesByPool(a),
          m = r.length;
        // console.log(m,'::::::::')
        $('#bar_lane_num').spinner('setValue', m);
        // $('#bar_lane_num2').spinner('setValue', m);
  ...
  //       var f = this.files[0].type,
  //         e = this.files[0].size;
  //       if ($.trim($(this).val()) == '') {
  //         return;
  //       }
  //       if (f.indexOf('image') < 0) {
  //         Util.globalTopTip('文件格式不正确，仅支持图像文件。', 'top_error', 2000, $('#image_dialog'), true);
  //         return;
  //       }
  //       if (e > 2097152) {
  //         Util.globalTopTip('图片大小不能超过2M', 'top_error', 2000, $('#image_dialog'), true);
  ...
  //         success: function (g) {
  //           $('#input_upload_image').val('');
  //           if (g.result == 'type_wrong') {
  //             $('#upload_img_res').html('此文件不是图片，请重新选择');
  //           } else {
  //             if (g.result == 'size_wrong') {
  //               $('#upload_img_res').html('文件大小超出要求，最大2M');
  //             } else {
  //               if (g.result == 'exception') {
  //                 $('#upload_img_res').html('无法使用此图片，请选择其他图片');
  //               } else {
  //                 if (g.result == 'svg_error') {
  //                   $('#upload_img_res').html('上传失败，svg文件中包含无法支持的foreignObject属性');
  //                 } else {
  //                   var h = g.image;
  //                   UI.setShapeImage(h.fileId, h.imageW || 100, h.imageH || 100);
  //                 }
  //               }
  //             }
  //           }
          ...
          for (var d = 0; d < j.length; d++) {
            ...
            for (var e = 0; e < k.length; e++) {
              ...
              if (l.size == 128 || e == k.length - 1) {
                ...
                var f = $(
                  "<div class='image_item' u='" + a + "' w='" + l.size_width + "' h='" + l.size_height + "'></div>",
                ...
                var b = $(
                  "<div class='drop_size'><span>" + l.size_width + 'x' + l.size_height + '</span></div>',
                ).appendTo(f);
                b.data('sizes', k);
                if (k.length > 1) {
  ...
  insertImage: function (a, k, d) {
    k = parseInt(k);
    d = parseInt(d);
    var e = $('#designer_layout');
    var c = e.width() / 2 + e.offset().left;
    var b = e.height() / 2 + e.offset().top;
    var i = Utils.getRelativePos(c, b, $('#designer_canvas'));
    var j = i.x.restoreScale() - k / 2,
      g = i.y.restoreScale() - d / 2;
    var f = Model.create('standardImage', j < 0 ? 0 : j, g < 0 ? 0 : g);
    f.props.w = k;
    f.props.h = d;
    f.fillStyle = { type: 'image', fileId: a, display: 'stretch', imageW: k, imageH: d };
    Model.add(f);
    Designer.painter.renderShape(f);
    Utils.unselect();
    Utils.selectShape(f.id);
    var l = Utils.getShapesBounding([f]);
    Designer.op.changeCanvas(l);
  },
  doExport: function (f) {
    ...
    if (c > 0 && f != 'pos') {
      ...
      html2canvas(h[0], { useCORS: true, backgroundColor: 'transparent', scale: 3 }).then(function (j) {
        ...
        $.ajax({
          ...
          type: 'post',
          data: { id: chartId, img: e.exportFile.eqDataUrl.src, source: 'equation' },
          success: function (k) {
  ...
  ieExportImg: function (c) {
    ...
    $('#export_type').val(c);
    if (c == 'svg' || c == 'pdfHD' || c == 'png' || c == 'print') {
      var g = flow2svg.init(c);
      $('#export_width').val(g.w);
      $('#export_height').val(g.h);
      var d = $('#svg_exporter')
        .html()
        .replace(/&nbsp;/g, '\u00a0');
      d = d.replace('xmlns="http://www.w3.org/2000/svg"', 'xmlns:xlink="http://www.w3.org/1999/xlink"');
      $('#export_definition').val(d);
      flow2svg.cleanSvgHtml();
    }
    if (c == 'print') {
      UI.print.svgCallback(d);
      return;
    }
    $.simpleAlert('close');
    ...
    $('#export_form').submit();
    try {
      buried_point_fn('click', 'export_result', '流程图编辑器', {
        操作: '下载文件',
        文件类型: 'pos',
        下载格式: c,
        操作结果: '成功',
  ...
  allExportImg: function (c) {
    ...
    $('#export_title').val($('.diagram_title').text() || '未命名文件');
    $('#export_type').val(c);
    if (c == 'svg' || c == 'pdfHD' || c == 'png' || c == 'image' || c == 'jpg' || c == 'print') {
      this.exportSVG(c);
    }
    $.simpleAlert('close');
    if (c == 'png' || c == 'image' || c == 'jpg' || c == 'print') {
      return;
    }
    $('#export_dialog').dlg('close');
    $('#export_form').submit();
  ...
  exportSVG: function (b) {
    ...
    if (b == 'png' || b == 'image' || b == 'jpg') {
      UI.downImg == true;
      $('#svg_exporter').html(
        '<canvas id="canvas_exporter" width="' + f.w * c + '" height="' + f.h * c + '"></canvas>',
      );
      canvg('canvas_exporter', d.replace(/\u00a0/g, '<tspan>\u00a0</tspan>'), {
        ...
        renderCallback: function () {
          if (UI.exportFile.errorNum > 0) {
            UI.exportFile.replaceErrorUrl(b);
            console.log('跨域图片处理');
            buried_point_fn('click', 'export_result', '流程图编辑器', {
              操作: '下载文件',
              文件类型: buried_point.visit.option['文件类型'],
      ...
      function e(i, j) {
        var h = new MouseEvent('click', { view: window, bubbles: false, cancelable: true });
  ...
  exportFile: {
    ...
    replaceErrorUrl: function (b) {
      var c = this;
      var a = c.errorUrls.map(function (d) {
      ...
      $.ajax({
        ...
        success: function (h) {
          var j = h.proUrls,
            d = j.length;
          for (var g = 0; g < d; g++) {
            if (c.errorUrls[g].indexOf('cdn.processon.com') != -1) {
              c.proUrlObj[c.errorUrls[g]] =
                'data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7';
            } else {
              c.proUrlObj[c.errorUrls[g]] = j[g];
            }
            if (!j[g]) {
              c._errorUrls.push(c.errorUrls[g]);
            }
          }
          for (key in Model.define.elements) {
            var e = Model.define.elements[key].fillStyle;
            if (e && e.fileId) {
              for (proUrlsKey in c.proUrlObj) {
                var f = e.fileId.replace(/^(http|https):\/\//, '');
                if (proUrlsKey.indexOf(f) != -1) {
                  e.fileId = c.proUrlObj[proUrlsKey];
                }
              }
            }
          }
          ...
          c.errorNum = 0;
          $('#svg-wrap').remove();
          if (c._errorUrls.length) {
            c.exportErrorTip(c._errorUrls[0]);
          } else {
            UI.exportSVG(b);
          }
        },
    ...
    exportErrorTip: function (b) {
      ...
      Util.globalTopTip(
        '下载失败，文件中存在不授信或者失效的图片,<br>请检查后重试，<a target="_blank" href="' + b + '">点击查看</a>',
        'top_error',
        8000,
        a,
        true,
  ...
  print: {
    data: {
      loading: false,
      svgPos: { w: 0, h: 0, minX: 0, minY: 0 },
      pageSize: { w: 0, h: 0 },
      printSize: { w: 1050, h: 1485 },
    },
    init: function () {
      var a = this.data;
      if (a.loading) {
      ...
      a.loading = true;
      Util.loading({ content: '打印加载中...', show: 10000, model: true });
      a.svgPos = $.extend({ w: 0, h: 0, minX: 0, minY: 0 }, Utils.restoreScale(flow2svg.calcCanvasSize()));
      a.svgPos.minX += 15;
      a.svgPos.minY += 15;
      a.pageSize.w = Model.define.page.width;
      a.pageSize.h = Model.define.page.height;
      a.printSize.w = 1050;
      a.printSize.h = 1485;
      if (Model.define.page.orientation == 'landscape') {
        a.pageSize.w = [a.pageSize.h, (a.pageSize.h = a.pageSize.w)][0];
        a.printSize.w = [a.printSize.h, (a.printSize.h = a.printSize.w)][0];
      }
      UI.doExport('print');
    },
    svgCallback: function (d) {
      var m = this.data.svgPos;
      var a = this.data.printSize;
      var h = this.data.pageSize;
      var j = $('<div>' + d + '</div>').css({
        width: a.w,
        height: a.h - 4,
        ...
        position: 'relative',
        overflow: 'hidden',
        backgroundColor: 'rgb(' + (Model.define.page.backgroundColor || '255,255,255') + ')',
        backgroundPosition: '10px -20px',
      });
      j.children().css({ width: '100%', height: '100%' });
      if (j.find('#pattern_mark').length) {
        j.find('#svgbackgroundid').remove();
        j.find('#pattern_mark').children().html('');
        var f = $('#watermarkCanvas')[0].toDataURL();
        j.css({ backgroundImage: 'url(' + f + ')' });
      }
      var n = Math.ceil(h.w / a.w);
      var g = Math.ceil(h.h / a.h);
      var e = '';
      for (var b = 0; b < g; b++) {
        for (var c = 0; c < n; c++) {
          var l = c * a.w - m.minX,
            k = b * a.h - m.minY;
          j.children().attr('viewBox', l + ' ' + k + ' ' + a.w + ' ' + a.h);
          e += j.prop('outerHTML');
        }
      }
      this.creatIframe(e);
    },
    creatIframe: function (g) {
      var d = this.data;
      var a = Model.define.page.orientation == 'landscape' ? 'landscape' : 'portrait';
      var c = Model.define.page.orientation == 'landscape' ? '3mm 4mm 0 4mm' : '6mm 4mm 0 4mm';
      var b =
        '<style>*{margin:0;padding:0} html,body{width:' +
        this.data.printSize.w +
        'px;-webkit-print-color-adjust:exact;color-adjust:exact;} @media print{@page{size:A4 ' +
        a +
        ';margin:' +
        c +
        ';}}</style>';
      $('#printIframe').length && $('#printIframe').remove();
      var f = $("<iframe id='printIframe'></iframe>")
        .css({ position: 'absolute', width: '0px', height: '0px', left: '-600px', top: '-600px' })
        .appendTo('body');
  ...
  loadEquation: function () {
    var c = {
      ...
      几何符号: ['\\Diamond', '\\Box', '\\triangle', '\\perp', '\\angle\\Alpha\\Beta\\Gamma', '60^\\circ'],
      戴帽符号: [
        '\\vec{c}',
        '\\overleftarrow{ab}',
        '\\overrightarrow{cd}',
      ...
      特殊符号: ['\\eth', '\\%', '\\dagger', '\\ddagger', '\\star', '*', '\\ldots', '\\smile', '\\frown', '\\wr'],
      '分数、矩阵和多行列式': [
        '\\frac{2}{4}=0.5',
        '{2 \\over 3}',
        '{{a+b} \\over {a-b}}',
    ...
    for (var b in c) {
      ...
      var d = '<h3>' + b + '</h3><div class="table"><div class="title"><span>效果</span><span>语法</span></div>';
      for (var a = 0; a < c[b].length; a++) {
        d += '<div><span>' + katex.renderToString(c[b][a]) + '</span><span class="copy">' + c[b][a] + '</span></div>';
      }
      d += '</div>';
    ...
    $('.eq-select li')
      ...
      .on('click', function () {
        var f = $(this);
        $('.eq-input').val(f.text());
        var e = document.getElementById('eq-container').getElementsByTagName('h3')[f.index()].offsetTop - 108;
  ...
  sendFeedBack: function (c) {
    ...
    Util.ajax({
      ...
      data: { content: b, username: $('#feedback_name').val(), email: a, url: location.href },
      success: function (d) {
        $('.dlg_mask').remove();
  ...
  showAddColla: function () {
    if (typeof collaboration == 'undefined') {
      var a = { css: ['/assets/css/collaboration.css'], js: ['/assets/js/collaboration.js'] };
    ...
    buried_point_fn('click', 'cooperate_show', '流程图编辑器', {
      弹窗名称: '协作',
      文件类型: buried_point.visit.option['文件类型'],
    });
  },
    ...
    $('#shape_manage_list')
      ...
      .each(function () {
        ...
        for (var d = 0; d < b.length; d++) {
          var e = b[d];
          if (
            (CategoryMapping.network && e == 'network') ||
            (CategoryMapping.ali_app_service && e == 'network_aliyun') ||
            (CategoryMapping.aws_analytics && e == 'network_aws') ||
            (CategoryMapping.cisco_bulidings && e == 'network_cisco') ||
            (CategoryMapping.azure && e == 'network_azure') ||
            (CategoryMapping.tencent_cloud_basic && e == 'network_tencentcloud') ||
            (CategoryMapping.aws2019_compute && e == 'network_aws2019') ||
            (CategoryMapping.aws2019_compute && e == 'network_aws2019')
          ) {
            f = true;
            break;
  ...
  showShareMenu: function (f) {
    if (navigator.userAgent.indexOf('app/tencent_wemeet') > -1) {
      ProShare.viewlink.wemeetExecute(chartId);
    } else {
      var a = $('#share_win');
      a.dialog();
      zhuge.track('点击分享按钮', { 触发位置: '流程图-分享' });
      ...
      d.find('li')
        ...
        .on('click', function () {
          ...
          b.empty();
          b.html(ProShare[g].source);
          ProShare[g].execute(chartId);
          var e = '创建链接';
          if (g == 'emb') {
            e = '嵌入到其他站点';
            buried_point_fn('click', 'share_click', '流程图编辑器', { 按钮名称: e });
          } else {
            if (g == 'image') {
              e = '生成在线图片';
              buried_point_fn('click', 'share_click', '流程图编辑器', { 按钮名称: e });
  ...
  showPublish: function (d) {
    ...
    return;
    poCollect('发布模版-点击', { 渠道记录: '编辑器内', 来源记录: '', 用户属性: '' });
    var a = $('#pubpo_win'),
      c = $('.pubpo-content');
    ...
    setTimeout(function () {
      a.dialog({
        onClose: function () {
          ...
          if (sessionStorage.getItem('published')) {
            sessionStorage.removeItem('published');
            $.ajax({
              url: '/templates/template_talent_check',
              success: function (f) {
                if (!f.isTalent && f.satisfy) {
                  b({
                    content:
                      '恭喜您，根据您的内容贡献，您已经获得模板"达人"申请资格，<br/>是否立即加入达人？获得更多官方加持。了解更多<a style="color:#4386F5" href="/ac/210319" target="_blank" onclick="poCollect(\'达人申请弹窗\',{\'渠道记录\':\'编辑器内\'});">"达人"权益。</a>',
                    okval: '立即加入',
                    cancelval: '暂不加入',
                    width: '500px',
                    title: 'ProcessOn邀请您加入"达人"',
                    onConfirm: function () {
                      poCollect('达人申请弹窗', { 渠道记录: '编辑器内' });
                      window.open('/ac/210319?from=editorPub');
                    },
                  });
                }
              },
    ...
    buried_point_fn('setTouchPlace', '流程图编辑器_发布');
    buried_point_fn('click', 'publish_show', '流程图编辑器', { 弹窗名称: '发布文件' });
  },
  ...
  doSaveAs: function () {
    ...
    Util.ajax({
      ...
      type: 'GET',
      data: { id: $('#hid_saveas_id').val(), ajaxCheck: 'ajaxCheck', team: teamId, org: orgId },
      success: function (a) {
        if (a.result == 'success') {
          $('#saveas_form').submit();
              ...
              if (tabCount != shape.path.length - 1) {
                ...
                for (var i = 0; i < tabCount; i++) {
                  ...
                  if (i < shape.textBlock.length) {
                    ...
                    newBlock.push({
                      position: { x: 'w/' + tabCount + '*' + i + '+5', y: 5, w: 'w/' + tabCount + '-10', h: 'h-10' },
                      text: 'Tab ' + (i + 1),
                    });
                  }
                }
          ...
          UI.showOptions(shape, [
            {
              ...
              onClick: function (btn) {
                ...
                if (shape.attribute.collapsed) {
                  ...
                  var w = shape.attribute.collapseW ? shape.attribute.collapseW : 400;
                  var h = shape.attribute.collapseH ? shape.attribute.collapseH : 300;
                  ...
                  for (var i = 0; i < rowArr.length - 1; i++) {
                    for (var j = 0; j < columnArr.length - 1; j++) {
                      ...
                      if (shape.textBlock[textNum]) {
                        text = shape.textBlock[textNum].text ? shape.textBlock[textNum].text : '';
                      }
              ...
              },
              {
                ...
                onChange: function (count) {
                  ...
                  if (count > column) {
                    ...
                    var wVal = columnArr[columnLen - 1] - columnArr[columnLen - 2];
  ...
  showOptions: function (f, m) {
    var e = $('#shape_opt_box');
    if (e.length == 0) {
      e = $("<div id='shape_opt_box'><div class='shape_opts'></div><div class='ico dlg_close'></div></div>").appendTo(
        '#designer_canvas',
      );
    ...
    e.css({ left: (h.x + h.w + 10).toScale(), top: h.y.toScale(), 'z-index': Model.orderList.length + 1 });
    ...
    for (var c = 0; c < m.length; c++) {
      ...
      if (a.type == 'spinner') {
        ...
        var k = $("<div class='spinner active' style='width: 55px;'></div>").appendTo(j);
        if (a.id) {
          k.attr('id', a.id);
        }
        k.spinner({ min: 1, max: typeof a.max != 'undefined' ? a.max : 20, step: 1, onChange: a.onChange });
        k.spinner('setValue', a.value);
      } else {
        if (a.type == 'button') {
          var b = $("<div class='button_box'></div>").appendTo(l);
          var d = $("<div class='toolbar_button active'>" + a.value + '</div>').appendTo(b);
  ...
  showThemeSelect: function () {
    ...
    if ($('#themes').children('.theme_box').length == 0) {
      for (var d in Schema.themes) {
        ...
        var e = $("<div theme='" + d + "' class='theme_box'><canvas width='130' height='130'></canvas></div>").appendTo(
          $('#themes'),
        );
        e.bind('click', function () {
  ...
  editMyshape: {
    ...
    renderMyshape: function (b, e) {
      ...
      d.find(':text').blur(function () {
        ...
        if (h.length > 20) {
          Util.globalTopTip('图形名称字符不能超过20', 'top_error', 2000, $('#myshape_dialog'), true);
          return;
        }
        $.ajax({
          ...
          success: function (i) {
            if (i.result == 'success') {
              e.title = h;
              Util.globalTopTip('修改成功', 'top_success', 2000, $('#myshape_dialog'), true);
            } else {
              Util.globalTopTip('保存失败', 'top_error', 2000, $('#myshape_dialog'), true);
            }
          },
        });
      });
          ...
          $.ajax({
            ...
            success: function (g) {
              ...
              if (g.result == 'success') {
                ...
                Util.globalTopTip('已删除', 'top_success', 2000, $('#myshape_dialog'), true);
              } else {
                Util.globalTopTip('保存失败', 'top_error', 2000, $('#myshape_dialog'), true);
              }
            },
    ...
    moveEvent: function () {
      ...
      $('.myshape_wrap')
        ...
        .on('mousedown.myshape', function (m) {
          ...
          if (m.target.tagName == 'INPUT' || m.target.tagName == 'I' || b.length == 0) {
            return;
          }
          f = b.index() - 1;
          c = b.index();
          ...
          $('.myshape_wrap')
            ...
            .on('mouseup.myshape', function (x) {
              ...
              if (x.target.tagName == 'INPUT' || x.target.tagName == 'I' || b.length == 0) {
                return;
              }
              $('.myshape_wrap').off('mousemove.myshape');
              a.remove();
                    ...
                    if (E.result == 'success') {
                      ...
                      if (f > c) {
                        h.each(function (H, I) {
                          if (H >= c && H < f) {
                            if (H % 5 == 4) {
                              $(I).css({ transform: 'translate(-528px,162px)' });
                            } else {
                              $(I).css({ transform: 'translate(132px,0px)' });
                            }
                          }
                        });
                        B.css({ transform: 'translate(' + 132 * F + 'px ,' + 162 * G + 'px)' });
                        h.css({ transition: 'transform 0.3s' });
                        setTimeout(function () {
                          D.before(B);
                          h.css({ transform: 'translate(0)', transition: 'none' });
                          g.isLoading = true;
                        }, 300);
                      } else {
                        h.each(function (H, I) {
...
var Dock = {
  init: function () {
    ...
    $('#dock_zoom').spinner({
      ...
      step: 10,
      onChange: function (f) {
        Designer.setZoomScale(f / 100, 'center');
      },
    });
    ...
    $('.dock_page_ori_list')
      ...
      .bind('click', function () {
        ...
        if (Model.define.page.showPrintLine) {
          Model.define.page.orientation = g;
          var f = Utils.getPrintPageSize();
          Designer.setPageStyle({ orientation: g, width: f.width, height: f.height });
          return;
        }
        Designer.setPageStyle({ orientation: g });
      });
    ...
    $('#dock_page_showPrintLine').bind('change', function () {
      var f = $(this).is(':checked');
      if (f) {
        var g = Utils.getPrintPageSize();
        Designer.setPageStyle({ showPrintLine: f, width: g.width, height: g.height });
      } else {
        Designer.setPageStyle({ showPrintLine: f });
      }
    });
    ...
    if (this.currentView == 'navigator') {
      ...
      if (this.currentView == 'graphic') {
        ...
        if (f == 0) {
          ...
          Dock.setFillStyle(k);
          $('#spinner_opacity').spinner('setValue', Math.round((a.shapeStyle.alpha / 1) * 100) + '%');
        }
      } else {
        if (this.currentView == 'metric') {
          ...
          if (f == 0) {
            ...
            if (a.category == 'lane') {
              ...
              $('#dock_metric_angle')
                .button('enable')
                .spinner('setValue', Math.round((a.props.angle / Math.PI) * 180) + '°');
            }
          }
        } else {
          if (this.currentView == 'page') {
            ...
            var o = i.width;
            var s = i.height;
            var b = $('#page_size_list').children('li[w=' + o + '][h=' + s + ']');
            var r = '';
            if (b.length > 0) {
              b.find('.ico_selected').remove();
              r = b.text();
            } else {
            ...
            $('#dock_page_showPrintLine').attr('checked', i.showPrintLine);
            if (i.showGrid) {
              $('#dock_gridsize_box').show();
            } else {
              $('#dock_gridsize_box').hide();
            ...
            var n = '';
            var q = $('#page_gridsize_list').children('li[s=' + i.gridSize + ']');
            if (q.length > 0) {
              var n = q.text();
            }
            ...
            if (typeof firstWatermarkSet == 'undefined') {
              if (i.watermark && i.watermark != '') {
                $('#watermark_input').val($('<span/>').html(i.watermark).text());
              }
  ...
  restoreVersion: function () {
    ...
    if (e.length) {
      CLB.saveVersion(null, Dock.currentDefinition);
      MessageSource.beginBatch();
      var a = Dock.currentDefinition.elements;
      var f = [];
      if (a) {
        for (var c in a) {
          f.push(a[c]);
      ...
      var g = { page: Utils.copy(Dock.currentDefinition.page), update: Utils.copy(Model.define.page) };
      MessageSource.send('updatePage', g);
      var b = Model.define.elements;
      var h = [];
      if (b) {
        for (var c in b) {
          h.push(b[c]);
      ...
      var i = e.attr('uid') + '_' + e.attr('vid') + '_' + e.attr('def');
      MessageSource.send('create', h, false, i);
      var d = { theme: Utils.copy(Dock.currentDefinition.theme), update: Utils.copy(Model.define.theme) };
      MessageSource.send('setTheme', d);
      MessageSource.commit();
      MessageSource.redoStack.stack = [];
      MessageSource.undoStack.stack = [];
      Designer.events.push('undoStackChanged', 0);
      Designer.events.push('redoStackChanged', 0);
  ...
  mindWaterMark: {
    data: { waterMarkUsed: true, isBluring: false, oldWaterName: '', waterMarkList: [], that: null, isMember: false },
    init: function () {
    ...
    eventInit: function () {
      ...
      $('#watermark_ullist').on('click', 'span.dock_btn_watermark_del', function (b) {
        b.stopPropagation();
    ...
    checkActived: function (b) {
      ...
      if (typeof b === 'string' && typeof c.data.oldWaterName === 'string' && c.data.oldWaterName === b) {
        return true;
      } else {
        return false;
      }
    ...
    toSetWaterMark: function (b, c, a) {
      ...
      if (a === 'add' && !d.checkSameWaterMark(c)) {
        if (d.data.waterMarkList.length >= 10) {
          UI.toast('最多可添加10条水印，如需添加新水印请先删除多余水印', 'center', 2000);
          return;
        }
        return;
      }
      ...
      this.api.getWaterMarks(function (e) {
        ...
        $(c.data.waterMarkList).each(function (g, f) {
          ...
          if (c.checkActived(f)) {
            ...
            d += '<span class="diagraming-icons" style=\'color:#333;\'>&#xe75c;</span>';
            d += '</div>';
          }
          d += f;
          d += '<div class="dock_operate fr">';
          d += '<span class="diagraming-icons dock_btn_watermark_del" _index=\'' + g + "'>&#xe637;</span>";
          d += '</div>';
          d += '</li>';
        });
  ...
  publish: {
    ...
    execute: function (g) {
      ...
      $('.change-tags')
        .off()
        .on('click', function () {
          var h = ProShare.publish.getRandomItem(ProShare.publish.officialTags, 5);
          b.empty();
      ...
      b.off().on('click', 'li', function (k) {
        ...
        if ($('#tag_items').children('span').length < 5) {
          j.remove();
          b.append('<li>' + ProShare.publish.getRandomItem(ProShare.publish.officialTags, 1)[0] + '</li>');
      ...
      $('#btn_submit_publish')
        ...
        .on('click', function () {
          ...
          d(i);
          buried_point_fn('click', 'publish_click', '流程图编辑器', { 按钮名称: '发布' });
        });
      $('#btn_submit_private')
        .off()
        .on('click', function () {
      ...
      function d(n) {
        ...
        o._public_clone_price = $('#setprice').val() ? $('#setprice').val() : $('.select-price-item.active').text();
        o.ignore = 'description';
        if (n != 'private') {
          if (o.title == '' || o.title == '未命名文件') {
            Util.globalTopTip('未命名文件不允许发布，请修改文件标题后再发布', 'top_error', 3000, $('#pubpo_win'), true);
            $('#pub_title').focus();
            return;
          }
          ...
          if (!o.industry) {
            Util.globalTopTip('请选择文件分类', 'top_error', 3000, $('#pubpo_win'), true);
            return;
          }
          if (o.tags.length == 0) {
          ...
          if (l) {
            ...
            if (k == 0 || k > 99) {
              Util.globalTopTip('请您输入价格(3 - 99)', 'top_error', 3000, $('#pubpo_win'), true);
              m.focus();
              return;
            } else {
              if (!/^(\d+\.\d{1,1}|\d+)$/.test(k)) {
                ...
                if (k < 3) {
                  Util.globalTopTip('克隆价格需大于3元', 'top_error', 3000, $('#pubpo_win'), true);
                  m.focus();
                  return;
                }
              }
        ...
        Util.ajax({
          ...
          success: function (p) {
            ...
            if (p.result == 'clone' || p.result == 'error_clone') {
              Util.globalTopTip('克隆的文件暂不允许发布', 'top_error', 3000, $('#pubpo_win'), true);
              a();
              buried_point_fn('click', 'publish_result', '流程图编辑器', {
                操作: '发布文件',
                文件名称: o.title,
                文件ID: o.id,
                分类: o.industry,
                标签: o.tags,
                文件类型: buried_point.visit.option['文件类型'],
                克隆类型: o._public_clone,
                价格: o._public_clone_price,
                发布结果: '失败',
                失败原因: '克隆的文件',
              });
              return;
              ...
              if (p.result == 'error_owner') {
                Util.globalTopTip('非自己文件，不允许发布', 'top_error', 3000, $('#pubpo_win'), true);
                a();
                buried_point_fn('click', 'publish_result', '流程图编辑器', {
                  操作: '发布文件',
                  文件名称: o.title,
                ...
                if (p.result == 'error_status') {
                  Util.globalTopTip('文件已在审核中，不允许发布', 'top_error', 3000, $('#pubpo_win'), true);
                  a();
                  buried_point_fn('click', 'publish_result', '流程图编辑器', {
                    操作: '发布文件',
                    文件名称: o.title,
                  ...
                  if (p.result == 'error_ticket') {
                    Util.globalTopTip('发布操作频繁，请操作后重试', 'top_error', 3000, $('#pubpo_win'), true);
                    a();
                    buried_point_fn('click', 'publish_result', '流程图编辑器', {
                      操作: '发布文件',
                      文件名称: o.title,
      ...
      function c(i) {
        ...
        if ($('#tag_items').children('span').length >= 5) {
          $('#tag_input').inputTip({ text: '最多添加五个标签', pos: 'rightout' });
          return;
        }
        if (i != '') {
    ...
    execute: function (j) {
      ...
      Util.get('/view/getlink', { chartId: j }, function (m) {
        ...
        if (locale == 'zh') {
          ...
          $('.share_to_a.weibo').attr('hrefs', $('.share_to_a.weibo').attr('hrefs').replace('$title', n)).show();
          $('.share_to_a.qzone').attr('hrefs', $('.share_to_a.qzone').attr('hrefs').replace('$title', n)).show();
          $('.share_to_a.mingdao')
            .attr('hrefs', $('.share_to_a.mingdao').attr('hrefs').replace('$thumb', h.thumb))
            .show();
          $('.share_to_a.douban').attr('hrefs', $('.share_to_a.douban').attr('hrefs').replace('$title', n)).show();
          ...
          $('.share_to_a.weibo').attr('hrefs', $('.share_to_a.weibo').attr('hrefs').replace('$title', n)).hide();
          $('.share_to_a.qzone').attr('hrefs', $('.share_to_a.qzone').attr('hrefs').replace('$title', n)).hide();
          $('.share_to_a.mingdao').attr('hrefs', $('.share_to_a.mingdao').attr('hrefs').replace('$thumb', n)).hide();
          $('.share_to_a.douban').attr('hrefs', $('.share_to_a.douban').attr('hrefs').replace('$title', n)).hide();
              ...
              if (p.viewLinkId != '') {
                ...
                if (k.hasClass('weixin')) {
                  ...
                  if ($('.share-to-weixin').length < 1) {
                    m = $("<div class='share-to-weixin'></div>").appendTo('body');
                    m.css({
                      position: 'absolute',
                      width: 200,
                      height: 216,
                    ...
                    m.find('img')
                      .css({ width: '180px', height: '180px', marginTop: 10, marginLeft: 10 })
      ...
      $('.switchbutton')
        ...
        .on('click', function () {
          ...
          if (k == 'true') {
            $(this).attr('val', false);
            buried_point_fn('click', 'share_click', '流程图编辑器', { 按钮名称: '删除密码' });
          } else {
            $(this).attr('val', true);
            buried_point_fn('click', 'share_click', '流程图编辑器', { 按钮名称: '添加密码' });
          }
          c(this);
        });
      $('.icons.link-icon')
        ...
        .on('click', function () {
          ...
          try {
            if (document.execCommand('copy', false, null)) {
              Util.globalTopTip('链接已复制到剪切板', 'top_success', 3000, $('#share_win'), true);
              buried_point_fn('click', 'share_click', '流程图编辑器', { 按钮名称: '复制链接' });
            } else {
            }
          } catch (k) { }
        });
      ...
      function g(k) {
        $('.dlg-content .create')
          ...
          .on('click', function () {
            f();
            $('.share-menu, .button-line').hide();
            buried_point_fn('click', 'share_click', '流程图编辑器', { 按钮名称: '删除链接' });
          });
        $('.share-menu, .button-line').show();
        $('#del-hint').show();
        b(k);
        buried_point_fn('click', 'share_click', '流程图编辑器', { 按钮名称: '创建链接' });
      }
      function b(k) {
        var m = $('.switchbutton');
        var l = m.attr('val');
    ...
    wemeetExecute: function (e) {
      ...
      function b(g, f) {
        wemeet.app
          ...
          .then(function (h) {
            console.log('setShareOpenAppConfigsucc', h);
          });
        wemeet.app.shareOpenApp().then(function (h) {
          console.log('shareOpenAppsucc', h);
        });
      }
    },
  },
  emb: {
    ...
    execute: function (g) {
      ...
      function e(i, l) {
        ...
        $('.embed-preview-wrap').css({ 'margin-top': -l / 2 + 'px', 'margin-left': -i / 2 + 'px' });
      ...
      $('.embed-size')
        .find('input')
        .keyup(function () {
          var i = $.trim($('#embed_width').val()) == '' ? 340 : $.trim($('#embed_width').val());
          var j = $.trim($('#embed_height').val()) == '' ? 160 : $.trim($('#embed_height').val());
          i = parseInt(i);
          j = parseInt(j);
          $('.embed-preview')
            .find('div:first')
...
var smartAiHelpCon = {
  ...
  init: function (n) {
    ...
    if (f.length < 1) {
      f = $("<div id='right-help-con' class='right-help-con " + (c.ai_class || '') + "'></div>")
        .appendTo('body')
        .hide();
      if (c.newclass) {
      ...
      f.append(g);
      var m = $("<div class='power-search-result' style='display: none'></div>");
      f.append(m);
      if (c.subUnit) {
        var a = c.subUnit,
          d = '';
    ...
    if ($(window).height() < f.outerHeight() + 140) {
      f.css('height', $(window).height() - 140);
    } else {
      f.css('height', c.height || f.outerHeight());
    }
  ...
  renderQuestion: function (b) {
    var a = 'https://ai.processon.com/doc/random/' + b.channel + '/' + b.designerType + '/' + b.questions;
    $.ajax({
      url: a,
      type: 'get',
      ...
      success: function (f) {
        if (f.result == 'success') {
          ...
          for (var c = 0; c < f.length; c++) {
            var e = f[c];
            d += "<div class='comm-item' desc = '" + e.desc + "' url = '" + e.photoUrl + "'>" + e.title + '</div>';
          }
          d += '</div>';
          if ($('#extra-box').length) {
            $('#extra-box').append(d);
  ...
  getHotCon: function (b) {
    ...
    var a = 'https://ai.processon.com/searchTag/' + b.channel + '/' + b.designerType;
    $.ajax({
      url: a,
      type: 'Post',
      success: function (f) {
  ...
  powerSearch: function (e) {
    ...
    var d = JSON.stringify({ keyword: e, designerType: c.params.designerType, channel: c.params.channel });
    $.ajax({
      url: b,
      type: 'POST',
      contentType: 'application/json',
            ...
            if (q.length) {
              for (var h = 0; h < q.length; h++) {
                ...
                if (h < c.params.resultNum) {
                  o.push("<div id='" + s.id + "' class='res-temp' tit='" + s.tit + "'>" + r + '</div>');
                }
              }
            }
            if (p.length) {
  ...
  tutorial: function (f, e, j, h) {
    ...
    if (c.length < 1) {
      c = $(
        '<div id="power-tutorial-con" class="tutorial-detail dialog-box"><h3></h3><div class="tutorial-image"></div><div class="tutorial-text"><div class="textarea"></div></div></div>',
      );
      c.appendTo('body');
    }
    var g = c.find('h3'),
  ...
  spread: function () {
    ...
    $('#extra-box').css('display', 'block');
    if ($(window).height() < a.outerHeight() + 140) {
      a.css('height', $(window).height() - 140);
    } else {
      a.css('height', 560);
    }
  },
...
(function (c) {
  c.fn.button = function (e) {
    if (typeof e == 'string') {
      if(c(this).selector == '#bar_font_family' && e == "setText"){
        // console.log('监听画布', '======')
        window.postMessage({name:"toShowItem"}, '*')
      }else if(c(this).selector == '#bar_font_family' && e != "setText"){
        window.postMessage({name:"toCloseItem"}, '*')
      }
      if (e == 'disable') {
  ...
  c.fn.dropdown = function (e) {
    ...
    if (typeof e == 'string') {
      if (e == 'close') {
        ...
        if (e == 'select') {
          arguments[1].prepend("<div class='ico ico_selected  diagraming-icons'>&#xe75c;</div>");
        }
      }
      return;
    }
  ...
  c.colorpicker = function (f) {
    ...
    var d = {
      ...
      setPancelColor: function (i) {
        ...
        e.find('.color_panel').css('background', 'rgba(' + g.r + ',' + g.g + ',' + g.b + ',' + this.data.rgba.a + ')');
      },
      setShowColor: function () {
        ...
        e.find('.color_show_now')
          .css('background', 'rgba(' + h.r + ',' + h.g + ',' + h.b + ',' + this.data.rgba.a + ')')
        ...
        switch (j) {
          ...
          default:
            var l = e.find('.input_r').val(),
              ...
              i = { r: l ? parseInt(l) : 0, g: k ? parseInt(k) : 0, b: h ? parseInt(h) : 0 };
      ...
      changeViewByHsb: function () {
        var h = this.data.hsb,
          ...
          g = j.width(),
          i = j.height();
        j.find('.picker_pin').css({ left: Math.round((h.s * g) / 100), top: Math.round(((100 - h.b) * i) / 100) });
    ...
    if (typeof e == 'string') {
      if (e == 'setColor') {
        if (arguments[1] == 'transparent') {
          d.children('.picker_btn_holder').css('background-color', 'transparent');
        } else {
  ...
  c.fn.spinner = function (g) {
    ...
    j.html(
      "<div class='spinner_input'><input/></div><div class='buttons'><div class='spinner_up' id='spinnerUp'></div><div class='spinner_down'></div></div>",
    );
  ...
  function a(j, h, g) {
    // console.log(document.getElementById('crossNum').style.display === 'none','dgshadghg')
    // console.log(j,h,g,'jkfjgkfjk')
    // if(document.getElementById('crossNum').style.display === 'none')
    //   j.attr('min',0);
    // else
    //   j.attr('min',2);
          ...
          if (d == 'isDisabled') {
            ...
            if (d == 'isSelected') {
              ...
              if (d == 'unselect') {
                ...
                if (d == 'select') {
                  return e.prepend("<div class='ico ico_selected diagraming-icons'>&#xe75c;</div>");
                }
              }
            }
          }
  ...
  window.callback_public = function (e) {
    ...
    buried_point_fn('click', 'publish_click', '流程图编辑器', { 按钮名称: '点击验证' });
  };
})(jQuery);
