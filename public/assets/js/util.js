...
(function () {
    ...
    $('[title],[original-title]')
      .live('mouseover', function () {
        ...
        if (!(0 < $('#mind_hover_tip').length)) {
          var t = $(this);
          if (
            (t.attr('title') && (t.attr('original-title', t.attr('title')), t.removeAttr('title')),
  ...
  fromUrl && fromUrl.indexOf('processon.com') < 0 && Util.setCookie('processon_referrer', encodeURI(fromUrl), 1),
    ...
    (Util.loginWindow = function (t, e) {
      if ((void 0 === t && (t = 'open'), 'open' == t)) {
        ...
        var i = $("<div id='loginWindow' style='margin-top:-120px;' class='loginWindow'></div>").appendTo('body');
        i.append(
          ...
          i.dialog({ onClose: function () {} });
      } else (t = 'close') && $('#loginWindow').dialog('close');
    }),
    (Util.payWindow = function (t, e, i) {
      if ((void 0 === t && (t = 'open'), 'open' == t)) {
        $('#payWindow').length && $('#payWindow').remove();
        var o = $("<div id='payWindow' style='margin-top:-120px;' class='payWindow'></div>").appendTo('body');
        o.append(
      ...
      } else (t = 'close') && ($('#weixinpay').dialog('close'), $('#payWindow').dialog('close'));
    });
  ...
  ($.mask = function (t) {
    if ((void 0 === t && (t = 'open'), 'open' == t)) {
      if (0 == maskStackCount) {
        ...
        e
          .css({ width: $(window).width() + 'px', height: $(window).height() + 'px', filter: 'alpha(opacity=60)' })
          .show(),
          $(window).bind('resize.mask', function () {
            e.css({ width: $(window).width() + 'px', height: $(window).height() + 'px' });
          });
      }
      maskStackCount++;
    ...
    ($.fn.dialog = function (o) {
      ...
      if ('string' == typeof o)
        'close' == o &&
          (n.find('.dialog-close').trigger('click'), null != $('#window-mask') && $('#window-mask').hide());
      else {
        (o = $.extend({ fixed: !0, closable: !0, mask: !0 }, o)) || (o = {});
        var t = '';
        o.title ? (t = o.title) : n.attr('title') && ((t = n.attr('title')), n.attr('title', '')),
          n.addClass('dialog-box').show();
        var e = $("<div class='dialog-close'></div>").appendTo(n);
        e.bind('click', function () {
          if (($(document).off('keyup.confirm'), !o.onClose || 0 != o.onClose())) {
            $.mask('close'), n.hide(), n.removeClass('dialog-box').find('.dialog-close').remove();
            var t = n.find('.dialog-title');
            n.attr('title', t.text()), t.remove(), $(window).unbind('resize.dialog');
          }
        }),
          ...
          o.mask && $.mask(),
          $(window).height() < n.outerHeight() && n.css('height', $(window).height() - 20),
          $(window)
            .off('resize.dialog')
            .bind('resize.dialog', function () {
              var t = n.outerWidth(),
                ...
                i = 0;
              i = o.fixed
                ? (n.css('position', 'fixed'), ($(window).height() - e) / 2 + 'px')
                : (n.css('position', 'absolute'), ($(window).height() - e) / 2 + $(document).scrollTop() + 'px');
              $(window).width();
      ...
      return (
        $(this)
          ...
          .on('mousedown.drag', function (t) {
            ...
            var a = $(this),
              l = 'string' == typeof p.target && 'default' == p.target ? a : p.target,
              s = t.pageX,
              r = t.pageY,
              d = l.offset().left,
              c = l.offset().top;
            p.clone &&
              ((l = a.clone().removeAttr('id').css('position', 'absolute').offset({ left: d, top: c })),
              'function' == typeof p.clone &&
                (p.clone.call(l, t),
                (d = 1 * l.css('left').replace('px', '')),
                (c = 1 * l.css('top').replace('px', ''))),
              p.opacity && l.css('opacity', p.opacity)),
              $(document).on('mousemove.drag', function (t) {
                a.hasClass('ondrag') ||
                  (a.addClass('ondrag'), p.clone && l.appendTo(a.parent()), p.start.call(a[0], t));
                var e = t.pageX - s + d,
                  i = t.pageY - r + c;
                if (p.bounding) {
                  ...
                  o < e &&
                    n < i &&
                    e < o + p.bounding.outerWidth() - l.outerWidth() &&
                    i < n + p.bounding.outerHeight() - l.outerHeight() &&
                    l.offset({ left: e, top: i });
                } else l.offset({ left: e, top: i });
                p.drag.call(a[0], t);
              }),
    ...
    ($.confirm = function (t) {
      ...
      t.okval && (i = t.okval),
        ...
        e
          ...
          .off()
          .on('click', function () {
            e.dialog('close'), t.onConfirm && t.onConfirm(), $(document).off('keyup.confirm');
          }),
        e
          .find('.cancelbtn')
          .off('click.cancel')
    ...
    ($.imgtoast = function (t) {
      ...
      t.tid && e.find('.button').attr('href', '/upgrade?tid=' + t.tid + '&addu=pay'), e.dialog();
    }),
    ($.warnTip = function (t, e) {
      var i = $('.warn-tip');
      if (((e || t.data) && (i = $('.warn-tip[data-info=' + (e || t.data) + ']')), 'string' != typeof t)) {
        var o = { content: '', tipIcon: !1, pos: 'top', closable: !0, delay: null, onClose: null, callback: null };
        $.extend(o, t), 0 < i.length && i.remove();
        var n = '';
        if (o.tipIcon) {
          var a = '&#xe672;';
          -1 < o.tipIcon.indexOf('&#') && (a = o.tipIcon), (n = "<div class='warn-icon icons'>" + a + '</div>');
        }
        var l = '';
        if (
          (o.data && (l = 'data-info=' + o.data),
          0 ==
            (i = $(
              "<div class='warn-tip' " +
                l +
                '>' +
                n +
                "<div class='mind-tip-text'>" +
                o.content +
                "</div><div class='mind-tip-close icons'>&#xe622;</div></div>",
            ).appendTo('body')).find('.mind-tip-close').length)
        )
          $("<span class='mind-dlog-close icons'>&#xe623;</span>").appendTo(dialogTitle);
        !1 === o.closable
          ? (i.find('.mind-tip-close').css('display', 'none'),
            o.delay &&
              ('undefined' != typeof clearWarnTipTimer && clearTimeout(clearWarnTipTimer),
              (clearWarnTipTimer = setTimeout(function () {
                i.remove();
              }, o.delay))))
          : i.find('.mind-tip-close').css('display', 'block'),
          i
            .children('.mind-tip-close')
            .off('click')
            .on('click', function () {
              i.remove(), o.onClose && o.onClose instanceof Function && o.onClose();
            }),
          setTimeout(function () {
            i.css({ top: '120px', bottom: 'auto' });
          }),
          o.callback && o.callback instanceof Function && o.callback();
      } else 'close' == t && 0 < i.length && i.find('.mind-tip-close').trigger('click');
    }),
    ($.fn.popMenu = function (t) {
      ...
      if ('string' != typeof t) {
        ...
        e.addClass('popover').css('z-index', i.zindex),
          ...
          i.autoClose &&
            ...
            $(document).bind('mouseup.popmenu', function () {
              e.popMenu('close'), $(document).unbind('mouseup.popmenu'), i.onClose && i.onClose();
            })),
        ...
        i.autoPosition && a + i.offsetY + e.outerHeight() > $(window).height() + $(document).scrollTop()
          ? e.css({ top: $(window).height() - e.outerHeight() + $(document).scrollTop(), left: n + i.offsetX })
    ...
    ($.fn.suggest = function (t) {
      var r = $(this),
        ...
        d = $.extend(e, t),
        c = $('.suggest-menu');
      c.length < 1 && (c = $("<ul class='suggest-menu'></ul>").appendTo('body')), c.width(d.width);
      var i = -1,
        p = '';
      r.off('keydown.suggest')
          ...
          if (40 == t.keyCode)
            t.preventDefault(),
              i < c.children().length - 1 &&
                (i++, c.find('.active').removeClass('active'), c.find('li[index=' + i + ']').addClass('active'));
            ...
            var e = c.find('.active');
            e.length && r.val(e.attr('val')), d.onEnter && d.onEnter(r), c.hide(), (value = '');
          }
        })
        .off('keyup.suggest')
        .on('keyup.suggest', function (t) {
          ...
          '' == s
            ...
            : s != p &&
              ((i = -1),
              $.get(d.url, { q: s }, function (t) {
                c.empty();
                var e = t.items;
                if (0 == e.length) c.hide(), (s = '');
                else {
                  for (var i = 0; i < e.length; i++) {
                    var o = e[i],
                      n = "<li index='" + i + "' class='suggest-item' val='" + o[d.valueField] + "'>";
                  ...
                  (a =
                    'center' == d.position
                      ? r.offset().left + r.outerWidth() / 2 - c.outerWidth() / 2
                      : 'right' == d.position
                      ? r.offset().left + r.outerWidth() - c.outerWidth()
                      : r.offset().left) +
                    c.outerWidth() >
                    $(window).width() && (a = $(window).width() - c.outerWidth());
                  var l = r.offset().top + r.outerHeight();
                  d.autoPosition && l + d.offsetY + c.outerHeight() > $(window).height() + $(document).scrollTop()
                    ? c.css({
                        top: $(window).height() - c.outerHeight() + $(document).scrollTop(),
                        left: a + d.offsetX,
                      })
                    : c.css({ top: l + d.offsetY, left: a + d.offsetX }),
                    c
                      .find('.suggest-item')
                      .off('mousedown')
    ...
    ($.fn.pagination = function (t, e, i, o) {
      if (!(e <= 1)) {
        ...
        if (n < e) {
          ...
          e - (l = 0 < t - r ? t - r : 1) < n && (l = e - n + 1);
          s = l + n - 1;
        }
        var d = '';
        (d += 1 < t ? "<a p='" + (t - 1) + "'>«</a>" : "<a class='disabled'>«</a>"),
          2 <= l && (d += "<a p='1'>1</a>"),
          3 <= l && (d += "<a class='disabled ellipsis'>...</a>");
        for (var c = l; c <= s && !(e < c); c++)
          d += c == t ? '<a class="disabled">' + c + '</a>' : "<a p='" + c + "'>" + c + '</a>';
        s <= e - 2
          ? (d += "<a class='disabled ellipsis'>...</a><a p='" + e + "'>" + e + '</a>')
          : s <= e - 1 && (d += "<a p='" + e + "'>" + e + '</a>'),
    ...
    ($.fn.multiInput = function (a, t, e) {
      ...
      if ('string' != typeof a || 'setVal' != a) {
        (a = $.extend(
          { text: '请在此输入对方ProcessOn账号邮箱或手机号，回车添加', autoComplete: !1, url: '', params: {} },
          a,
        )),
          r.html('');
        var i = $('<div class="multi-input-vals"></div>'),
          o = $('<div><input type="text" id="multi-input" placeholder="' + a.text + '" style="width:96%"></div>');
        r.append(i).append(o),
          o
            .find('input')
            .off()
              ...
              if ('' != e)
                if (13 == t.keyCode && a.setVal) {
                  ...
                  for (var n = 0; n < e.length; n++)
                    if (((i = e[n].isEmail()), (o = /^[1][3,4,5,6,7,8,9][0-9]{9}$/.test(e[n])), !i && !o))
                    ...
                    $.ajax({
                      ...
                      success: function (t) {
                        ...
                        if (($('.popWindow').remove(), '' != e)) {
                          var i = $("<div class='popWindow'></div>").appendTo('body');
      ...
      function l(t, e) {
        ...
        if (e instanceof Array)
          for (var l = 0; l < e.length; l++) {
            if (((i = e[l].isEmail()), (o = /^[1][3,4,5,7,8][0-9]{9}$/.test(e[l])), i)) (a = '&#xe614;'), (t = e);
            else if (o) (a = '&#xe709;'), (t = e);
            else if (!i && !o) return;
            var s =
              '<span val="' +
    ...
    ($.fn.popWindow = function (t) {
      ...
      (o = 'center' == t.dropCenter ? i.offset().left - 0 + i.width() / 2 : i.offset().left),
        e
          .css({ left: o, top: i.offset().top + i.height() + (t.mh || 0), zIndex: t.index || 1 })
          .show()
          .siblings('.popWindow')
          .hide(),
    ...
    ($.fn.submitForm = function (opt) {
      ...
      function submitCallback() {
        ...
        var body = $('#' + frameId)
            ...
            .find('body'),
          data = body.html();
        if ('' == data) return --checkCount ? void setTimeout(submitCallback, 200) : void 0;
        ...
        try {
          ...
          options.json
            ? ($.simpleAlert('暂时无法处理您的请求，请稍候重试。', 'error', 3e3), options.error && options.error(data))
            : options.success && options.success(data);
        }
        setTimeout(function () {
    ...
    ($.fn.submitFormAjax = function (e) {
      ...
      (e.onSubmit && 0 == e.onSubmit.call()) ||
        $.ajax({
          ...
          error: function (t) {
            $.simpleAlert('暂时无法处理您的请求，请稍候重试', 'error', 3e3), e.error && e.error(t);
          },
        });
    }),
    ($.fn.numberTip = function (t) {
      var e = $.extend({ val: '+1', size: 14, color: 'red', time: 1e3, pos: 'right' }, t),
        i = $(this),
      ...
      if ('string' != typeof t && !(e.length <= 0)) {
        var i = $(e[0]),
          ...
          c =
            ...
            { w: i.width() + n + a, h: i.height() + l + s, ml: 10, mt: 10, maxWidth: o.width(), s: 150 }),
          p = $.extend(c, t),
          f = p.w,
          u = p.h,
          g = p.ml,
  ...
  $.fn.streamInput = function (t) {
    if (this[0] && t.face && 'DIV' == this[0].nodeName && !this.attr('stream_id')) {
      ...
      (o.id = n.length ? streamInputStreams[n[n.length - 1]].id + 1 : 1),
        (o.stream_id = 'stream_' + (n.length ? streamInputStreams[n[n.length - 1]].id + 1 : 1)),
        this.attr({ contentEditable: 'true', spellcheck: 'false', accesskey: 'q', stream_id: o.stream_id }),
      ...
      $(i.target)
              ...
              if ((h($(this)), m($(this)), p)) {
                ...
                if (l)
                  for (s.innerHTML = l; (n = s.firstChild); )
                    (a = r.appendChild(n)), 'SPAN' == n.nodeName && (f = r.childNodes.length + n.childNodes.length - 1);
                if ((o.insertNode(r), a)) {
                  if (
                    ...
                    (n = this.childNodes[d]), o.setStart(n, n.data.length - g), o.setEnd(n, n.data.length - g);
                  }
        ...
        $(i.target)
          ...
          .on('paste.stream', function (t) {
            ...
            o.deleteContents();
            try {
              var n = e.getData('text/html') || e.getData('text/plain').replace(/</g, '&lt;').replace(/>/g, '&gt;');
              n = n.replace(/<(\/)?(html|body)(\s|\S)+?>/g, '').replace(/(<!--.+?-->)|(\r\n)/g, '');
              var a = $('<div>' + n + '</div>');
              ...
              (l.className = 'paste-cont'),
                o.insertNode(l),
                (o = o.cloneRange()).selectNodeContents(l),
                i.removeAllRanges(),
                i.addRange(o);
              var s = (function (t) {
                var e = t.cloneNode(!0),
        ...
        $(i.face)
          ...
          .on('click.stream', function () {
            if (
              (((curr_stream_icon = streamInputStreams[$(this).attr('for_stream')]).target = $(
            ...
            e.length < 1 &&
              (e = $('<div></div>')
                ...
                .appendTo('body')),
              $('#faces_dialog').popMenu({ target: this, position: 'right', zindex: 4 }),
              $('#faces_dialog ul li')
                .off('click.stream')
                .on('click.stream', function (t) {
                  ...
                  if (
                    ((e = "<img class='ico-face' src='/assets/images/faces/faces/" + ($(this).index() + 1) + ".gif'>"),
    ...
    function r(t, e, i, o) {
      var n,
        ...
        s = document.createElement('div'),
        r = document.createDocumentFragment();
      if (l) for (s.innerHTML = l; (n = s.firstChild); ) a = r.appendChild(n);
      return (
        i.insertNode(r),
        a && ((i = i.cloneRange()).setStartAfter(a), i.collapse(!0), e.removeAllRanges(), e.addRange(i), $(o).keyup()),
        i
      );
    }
  };
    ...
    if (!e.onSend || 0 != e.onSend()) {
      return (
        ...
        $.ajax({
          ...
          success: function (t) {
            if ('error' == t.error)
              $.simpleAlert("<@i18n resource='global.error'/>", 'error', 3e3), e.error && e.error(t);
              ...
              if ((console.info('安全校验'), 'undefined' != typeof TencentCaptcha))
  ...
  (Util.globalTopTip = function (t, e, i, o, n) {
    if (void 0 !== t) {
      ...
      0 < a.length && a.remove(),
        ...
        n && (a.find('.left_arrow').remove(), a.find('.right_arrow').remove(), a.addClass('noarrow'));
      var l = a.outerWidth();
      o ? a.css('top', $(o).offset().top + 'px') : 0 == $('#header').length && a.css('top', '0px'),
        a.css({ 'margin-left': -0.5 * (parseInt(l) - 12) + 'px' }).show(),
        setTimeout(function () {
          a.addClass('show'),
            setTimeout(function () {
              a.removeClass('show'),
  ...
  (Util.globalLeftTip = function (t) {
    ...
    if (void 0 !== e.content) {
      ...
      0 < i.length && i.remove(),
        (i = $('<div id="global-leftbot-dialog" class="global-leftbot-dialog">' + e.content + '</div>').appendTo(
          'body',
        )).addClass(e.type),
        i.show(),
        setTimeout(function () {
  ...
  (Util.loading = function (t) {
    ...
    (t = $.extend({ content: 'loading...', show: 1e3, delay: 0, model: !1 }, t)),
        ...
        t.member
          ...
          ? (zhuge.track('文件不足弹窗'),
            $.confirm({
              ...
              onConfirm: function () {
                try {
                  buried_point_fn('setTouchPlace', '文件不足弹窗_去升级');
                } catch (t) {}
                e(!1);
              },
              okval: '升级扩容',
  ...
  (Util.filterXss = function (t) {
    return null == t || '' == t
      ? ''
      : (t = (t = (t = (t = (t = (t = (t = t.toString()).replace(/</g, '&lt;')).replace(/%3C/g, '&lt;')).replace(
          />/g,
          '&gt;',
        )).replace(/%3E/g, '&gt;')).replace(/'/g, '&apos;')).replace(/"/g, '&quot;'));
  }),
  (Util.restoreXss = function (t) {
    return null == t || '' == t
      ? ''
      : (t = (t = (t = (t = (t = t.replace(/&amp;/g, '&')).replace(/&lt;/g, '<')).replace(/&gt;/g, '>')).replace(
          /&apos;/g,
          "'",
        )).replace(/&quot;/g, '"'));
  }),
  ...
  (String.prototype.isEmail = function () {
    return !this.isEmpty() && !!/^\w+((-\w+)|(\.\w+))*\@[A-Za-z0-9]+((\.|-)[A-Za-z0-9]+)*\.[A-Za-z0-9]+$/.test(this);
  }),
  ...
  (Util.shapesCount = function (t) {
    1e3 < $('#designer_canvas').find('.shape_box').length &&
      null != UI &&
      UI.showTip('本文件的图形数量已足够绘制一艘航空母舰，为了好的作图体验，建议您不要制作过大的文件哦');
  }),
  (Util.GetNextMonthDay = function (t) {
    ...
    12 < o && ((i = parseInt(i) + parseInt(parseInt(o) / 12 == 0 ? 1 : parseInt(o) / 12)), (o = parseInt(o) % 12));
    var n = e.getDate(),
      a = new Date(i, o, 0);
    return (a = a.getDate()) < n && (n = a), o < 10 && (o = '0' + o), i + '年' + o + '月' + n + '日';
  }),
  (Util.GetNextDay = function (t, e) {
    if (e) var i = new Date(e);
    else i = new Date();
    var o = new Date(i);
    return o.setDate(i.getDate() + t), o.getFullYear() + '年' + (o.getMonth() + 1) + '月' + o.getDate() + '日';
  }),
  (Util.numAbb = function (t) {
    return t < 1e3
      ? t
      : 1e7 <= t
      ? '<span class="num_n">' + (t / 1e7).toFixed(2) + '</span><span class="num_unit">kw</span>'
      : 1e6 <= t
      ? '<span class="num_n">' + (t / 1e6).toFixed(2) + '</span><span class="num_unit">bw</span>'
      : 1e4 <= t
      ? '<span class="num_n">' + (t / 1e4).toFixed(2) + '</span><span class="num_unit">w</span>'
      : 1e3 <= t
      ? '<span class="num_n">' + (t / 1e3).toFixed(2) + '</span><span class="num_unit">k</span>'
      : void 0;
  }),
  (Util.toFixed = function (t, e) {
    t = t.toFixed(e);
    return '0' == (t += '')[t.length - 1] && 1 <= e ? Util.toFixed(parseFloat(t), e - 1) : parseFloat(t);
  }),
  (Util.strEllipsis = function (t, e) {
    return 'string' != typeof t ? '' : t.length > e ? t.substring(0, e) + '...' : t;
  }),
  ...
  (Util.norm_time = function (t) {
    ...
    var o = new Date(t);
    return (
      (oYear = o.getFullYear()), (oMonth = o.getMonth() + 1), (oDay = o.getDate()), oYear + '-' + oMonth + '-' + oDay
    );
  });
