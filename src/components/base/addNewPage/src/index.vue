...
import BoxFileOne from './messageBox/BoxFile1.vue';
import BoxFileTwo from './messageBox/BoxFile2.vue';
  ...
  methods: {
    ...
    addLine(bl = true) {
      ...
      dataJSON.elements['wjcdesxydd' + num] = {
        ...
        fontStyle: { orientation: 'horizontal', textAlign: 'center' },
    ...
    delLine(bl = true) {
      ...
      for (let i = 0, length = selectedSxyd.length; i < length - 1; i++) {
        for (let j = 0; j < length - i - 1; j++) {
          if (selectedSxyd[j].split('_')[selectedSxyd[j].split('_').length - 1] < selectedSxyd[j + 1].split('_')[selectedSxyd[j + 1].split('_').length - 1]) {
    ...
    toGetMapInfo() {
      ...
      // console.log(oldDdata,"ghgjhjhjhj888");
      ...
      for (let i = 0, length = mapData.length; i < length; i++) {
        ...
        if (!dataName && mapData[i].category !== 'lane' && !this.queryType) {
    ...
    sureAddWork(list) {
      ...
      window.onmessage = (data) => {
             console.log('2222');
        if (data.data.type === 'returnShow') {
          console.log(data.data.iframeMessage);
          if (this.showItem) {
            this.itemMessage.workFireList = this.itemMessage.workFireList.concat(data.data.iframeMessage);
          } else {
            this.flowMessage.workFireList = this.flowMessage.workFireList.concat(data.data.iframeMessage);
          }
        }
      };
      window.parent.postMessage({ iframeMessage: fd, type: 'uploadFile' }, '*');
    ...
    sureAddPoint(list1) {
      ...
      for (let i = 0, length = list1.length; i < length; i++) {
        fd.append('files', list1[i]);
      }
      window.onmessage = (data) => {
             console.log('2222');
        if (data.data.type === 'returnShow2') {
          console.log(data.data.iframeMessage);
          if (this.showItem) {
            this.itemMessage.knowFireList = this.itemMessage.knowFireList.concat(data.data.iframeMessage);
          } else {
            this.flowMessage.knowFireList = this.flowMessage.knowFireList.concat(data.data.iframeMessage);
          }
        }
      };
      window.parent.postMessage({ iframeMessage: fd, type: 'uploadFile2' }, '*');
